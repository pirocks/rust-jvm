...
use crate::stack_entry::StackEntryRef;
...
pub fn run_native_special_new<'vm>(jvm: &'vm JVMState<'vm>, int_state: Option<&mut InterpreterStateGuard<'vm, '_>>, method_id: MethodId, return_to_ptr: *const c_void) -> IRVMExitAction {
    ...
    //todo dup
    let args = setup_native_special_args(jvm, method_id, &method_view, current_frame);
...
pub fn setup_native_special_args<'gc>(jvm: &'gc JVMState<'gc>, method_id: MethodId, method_view: &MethodView, current_frame: StackEntryRef<'gc,'_>) -> Vec<NewJavaValueHandle<'gc>> {
...
pub fn run_native_static_new<'vm>(jvm: &'vm JVMState<'vm>, int_state: Option<&mut InterpreterStateGuard<'vm, '_>>, method_id: MethodId, return_to_ptr: *const c_void) -> IRVMExitAction {
    ...
    let current_frame = int_state.current_frame();
    let args = setup_static_native_args(jvm, method_id, &method_view, current_frame);
...
pub fn setup_static_native_args<'gc>(jvm: &'gc JVMState<'gc>, method_id: MethodId, method_view: &MethodView, current_frame: StackEntryRef<'gc,'_>) -> Vec<NewJavaValueHandle<'gc>> {
    let mut args = vec![];
    let memory_layout = NativeStackframeMemoryLayout { num_locals: jvm.num_local_vars_native(method_id) };
    let mut i = 0;
    for arg_type in method_view.desc().arg_types.iter() {
