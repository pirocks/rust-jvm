...
use early_startup::{
    EXTRA_LARGE_REGION_BASE, EXTRA_LARGE_REGION_SIZE, EXTRA_LARGE_REGION_SIZE_SIZE, LARGE_REGION_BASE,
    LARGE_REGION_SIZE, LARGE_REGION_SIZE_SIZE, MAX_REGIONS_SIZE_SIZE, MEDIUM_REGION_BASE,
    MEDIUM_REGION_SIZE, MEDIUM_REGION_SIZE_SIZE, Regions, SMALL_REGION_BASE,
    SMALL_REGION_SIZE, SMALL_REGION_SIZE_SIZE,
};
use jvmti_jni_bindings::{jdouble, jint, jlong, jobject, jvalue};
use rust_jvm_common::compressed_classfile::{
    CFieldDescriptor, CMethodDescriptor, CompressedParsedDescriptorType, CPDType, CPRefType,
};
use rust_jvm_common::compressed_classfile::code::{
    CInstruction, CompressedCode, CompressedInstruction, CompressedInstructionInfo, CompressedLdcW,
};
use rust_jvm_common::compressed_classfile::names::{
    CClassName, CompressedClassName, FieldName, MethodName,
};
...
use crate::class_loading::{
    assert_loaded_class, check_initing_or_inited_class, check_loaded_class_force_loader,
};
use crate::gc_memory_layout_common::{
    AllocatedObjectType, ArrayMemoryLayout, FramePointerOffset, MAGIC_1_EXPECTED, MAGIC_2_EXPECTED,
    MemoryRegions, ObjectMemoryLayout, StackframeMemoryLayout,
};
...
use crate::jit::{
    ByteCodeOffset, CompiledCodeID, IRInstructionIndex, LabelName, MethodResolver,
    NotSupported, ToIR, ToNative, transition_stack_frame, TransitionType, VMExitType,
};
...
pub struct JITedCodeState {
    ...
    address_to_byte_code_compressed_code:
    HashMap<CompiledCodeID, BiRangeMap<*mut c_void, CInstruction>>,
}
...
impl JITedCodeState {
    pub fn new() -> Self {
        ...
        let addr =
            BASE_CODE_ADDRESS + (thread_id_numeric.get() as usize) * THREAD_CODE_ADDRESS_MULTIPLIER;
        let res_code_address = unsafe {
            mmap(
                addr as *mut c_void,
                MAX_CODE_SIZE,
                ProtFlags::PROT_WRITE | ProtFlags::PROT_EXEC,
                MapFlags::MAP_ANONYMOUS | MapFlags::MAP_NORESERVE | MapFlags::MAP_PRIVATE,
                -1,
                0,
            )
                .unwrap()
    ...
    pub fn ip_to_bytecode_pc(
        &self,
        instruct_pointer: *mut c_void,
    ) -> Result<(u16, ByteCodeOffset), Opaque> {
        ...
        let address_to_bytecode_for_this_method = self
            .address_to_byte_code_offset
            .get(&compiled_code_id)
            .unwrap();
        let address_to_bytecode_index_for_this_method = self
            .address_to_byte_code_index
            .get(&compiled_code_id)
            .unwrap();
        let bytecode_offset = address_to_bytecode_for_this_method
            .get(&instruct_pointer)
            .unwrap();
        let index_offset = address_to_bytecode_index_for_this_method
            .get(&instruct_pointer)
            .unwrap();
        Ok((*index_offset, *bytecode_offset))
    }

    pub fn ip_to_bytecode_pcs(
        &self,
        instruct_pointer: *mut c_void,
    ) -> Result<Vec<ByteCodeOffset>, Opaque> {
        //todo track opaque funcitons
        let compiled_code_id = self.function_addresses.get(&instruct_pointer).unwrap();
        if self.opaque.contains(compiled_code_id) {
            return Err(Opaque {});
    ...
    fn add_top_level_exit_code(&mut self) -> *mut c_void {
        ...
        let ir = ToIR {
            ...
            ir: vec![
                ...
                (
                    ...
                    IRInstr::VMExit {
                        exit_label,
                        exit_type: VMExitType::TopLevelReturn {},
                    },
                    nop,
                ),
            ],
        ...
        self.add_from_ir(
            "top level exit wrapper function".to_string(),
            current_code_id,
            ir,
        )
    }
    ...
    pub fn add_function(
        ...
        let cinstructions = instructions
            .iter()
        ...
        let (current_rc, method_i) = resolver
            .jvm
            .method_table
            .read()
            .unwrap()
        ...
        let class_name = view
            ...
            .unwrap_name()
            .0
            .to_str(&resolver.jvm.string_pool);
        let res_ptr = self.add_from_ir(
            format!("{} {} {:?}", class_name, method_name, current_code_id),
            current_code_id,
            ir,
        );
        res_ptr
    ...
    fn to_ir<'l>(
        &mut self,
        byte_code: Vec<&CInstruction>,
        layout: &dyn StackframeMemoryLayout,
        resolver: MethodResolver<'l>,
    ) -> Result<ToIR, NotSupported> {
        let mut labels = vec![];
        ...
        let mut pending_labels = vec![
            (ByteCodeOffset(0), function_start_label),
            (
                ByteCodeOffset(byte_code.last().unwrap().offset),
                function_end_label,
            ),
        ];
        for (i, byte_code_instr) in byte_code.iter().enumerate() {
            ...
            match &byte_code_instr.info {
                ...
                CompressedInstructionInfo::goto_(offset) => {
                    ...
                    pending_labels.push((
                        ByteCodeOffset((current_offset.0 as i32 + *offset as i32) as u16),
                        branch_to_label,
                    ));
                    initial_ir.push((
                        current_offset,
                        IRInstr::BranchToLabel {
                            label: branch_to_label,
                        },
                    ));
                }
                CompressedInstructionInfo::ifnull(offset) => {
                    ...
                    initial_ir.push((
                        current_offset,
                        IRInstr::BranchEqual {
                            a: register_with_null,
                            b: possibly_null_register,
                            label: branch_to_label,
                        },
                    ))
                }
                ...
                CompressedInstructionInfo::iload_0 => {
                    JITedCodeState::gen_iload_n(
                        layout,
                        &mut initial_ir,
                        current_offset,
                        current_byte_code_instr_count,
                        next_byte_code_instr_count,
                        0,
                    );
                }
                ...
                CompressedInstructionInfo::iconst_0 => JITedCodeState::gen_iconst_n(
                    layout,
                    &mut initial_ir,
                    current_offset,
                    next_byte_code_instr_count,
                ...
                CompressedInstructionInfo::iconst_1 => JITedCodeState::gen_iconst_n(
                    layout,
                    &mut initial_ir,
                    current_offset,
                    next_byte_code_instr_count,
                ...
                CompressedInstructionInfo::iconst_2 => JITedCodeState::gen_iconst_n(
                    layout,
                    &mut initial_ir,
                    current_offset,
                    next_byte_code_instr_count,
                    ...
                    match resolver.lookup_type_loaded(&cpd_type) {
                        ...
                        Some((rc, _)) => {
                            let (field_number, field_type) =
                                rc.unwrap_class_class().field_numbers.get(name).unwrap();
                            ...
                            initial_ir.push((
                                ...
                                IRInstr::LoadFPRelative {
                                    from: layout
                                        .operand_stack_entry(current_byte_code_instr_count, 0),
                            ...
                            initial_ir.push((
                                current_offset,
                                IRInstr::Load {
                                    from_address: class_ref_register,
                                    to: to_get_field,
                                },
                            ));
                            initial_ir.push((
                            ...
                            initial_ir.push((
                                current_offset,
                                IRInstr::VMExit {
                                    exit_label: npe_exit_label,
                                    exit_type: VMExitType::Todo {},
                                },
                            ));
                            initial_ir.push((
                                current_offset,
                                IRInstr::Label(IRLabel {
                                    name: after_npe_label,
                                }),
                            ))
                        }
                    }
                }
                CompressedInstructionInfo::aconst_null => JITedCodeState::gen_code_aconst_null(
                    layout,
                    &mut initial_ir,
                    current_offset,
                    next_byte_code_instr_count,
                ...
                CompressedInstructionInfo::anewarray(cpdtype) => self.gen_code_anewarray(
                    layout,
                    &mut labels,
                    &mut initial_ir,
                    current_offset,
                ...
                CompressedInstructionInfo::new(class_name) => self.gen_code_new(
                    layout,
                    resolver,
                    &mut labels,
                    &mut initial_ir,
                ...
                CompressedInstructionInfo::ldc(Either::Left(left_ldc)) => self.gen_code_left_ldc(
                    &mut labels,
                    &mut initial_ir,
                    layout,
                    current_offset,
                    ...
                    match resolver.lookup_special(
                        ...
                        *method_name,
                        ...
                        Some((method_id, is_native)) => {
                            ...
                            let callee_this_pointer_offset = layout.operand_stack_entry(
                                current_byte_code_instr_count,
                                descriptor.arg_types.len() as u16,
                            );
                            ...
                            initial_ir.push((
                                current_offset,
                                IRInstr::LoadFPRelative {
                                    from: callee_this_pointer_offset,
                                    to: this_pointer,
                                },
                            ));
                            ...
                            initial_ir.push((
                                current_offset,
                                IRInstr::GrowStack {
                                    amount: next_function_layout.full_frame_size(),
                                },
                            ));
                            ...
                            initial_ir.push((
                                ...
                                IRInstr::StoreFPRelative {
                                    from: rip_register,
                                    to: FramePointerOffset(prev_rip_position),
                                },
                            ));
                            let prev_rbp_position =
                                layout.full_frame_size() + size_of::<*mut c_void>();
                            let prev_rbp_register = Register(1);
                            initial_ir.push((
                                current_offset,
                                IRInstr::LoadRBP {
                                    to: prev_rbp_register,
                                },
                            ));
                            initial_ir.push((
                                current_offset,
                                IRInstr::StoreFPRelative {
                                    from: prev_rbp_register,
                                    to: FramePointerOffset(prev_rbp_position),
                                },
                            ));
                            let frame_info_ptr =
                                layout.full_frame_size() + 2 * size_of::<*mut c_void>();
                            let beafbeaf_register = Register(2);
                            initial_ir.push((
                                current_offset,
                                IRInstr::Const64bit {
                                    to: beafbeaf_register,
                                    const_: 0xbeafbeafbeafbeaf,
                                },
                            ));
                            initial_ir.push((
                                current_offset,
                                IRInstr::StoreFPRelative {
                                    from: beafbeaf_register,
                                    to: FramePointerOffset(frame_info_ptr),
                                },
                            ));
                            ...
                            initial_ir.push((
                                current_offset,
                                IRInstr::StoreFPRelative {
                                    from: method_id_register,
                                    to: FramePointerOffset(debug_ptr),
                                },
                            ));
                            ...
                            initial_ir.push((
                                ...
                                IRInstr::Const64bit {
                                    to: magic_1_register,
                                    const_: MAGIC_1_EXPECTED,
                                },
                            ));
                            initial_ir.push((
                                current_offset,
                                IRInstr::StoreFPRelative {
                                    from: magic_1_register,
                                    to: FramePointerOffset(magic_1),
                                },
                            ));
                            ...
                            initial_ir.push((
                                ...
                                IRInstr::Const64bit {
                                    to: magic_2_register,
                                    const_: MAGIC_2_EXPECTED,
                                },
                            ));
                            initial_ir.push((
                                current_offset,
                                IRInstr::StoreFPRelative {
                                    from: magic_2_register,
                                    to: FramePointerOffset(magic_2),
                                },
                            ));
                            let local_vars_start =
                                layout.full_frame_size() + 6 * size_of::<*mut c_void>();
                            let temp_arg_register = Register(5);
                            for i in local_vars_start..descriptor.arg_types.len() {
                                let load_from_location = layout
                                    .operand_stack_entry(current_byte_code_instr_count, i as u16);
                                let load_to_location =
                                    FramePointerOffset(local_vars_start + i * size_of::<jlong>());
                                initial_ir.push((
                                    current_offset,
                                    IRInstr::LoadFPRelative {
                                        from: load_from_location,
                                        to: temp_arg_register,
                                    },
                                ));
                                initial_ir.push((
                                    current_offset,
                                    IRInstr::StoreFPRelative {
                                        from: temp_arg_register,
                                        to: load_to_location,
                                    },
                                ));
                            }
                            //pop this last
                            let load_from_location = layout.operand_stack_entry(
                                current_byte_code_instr_count,
                                descriptor.arg_types.len() as u16,
                            );
                            let load_to_location = FramePointerOffset(
                                local_vars_start + descriptor.arg_types.len() * size_of::<jlong>(),
                            );
                            initial_ir.push((
                                current_offset,
                                IRInstr::LoadFPRelative {
                            ...
                            initial_ir.push((
                                ...
                                IRInstr::VMExit {
                                    exit_label,
                                    exit_type: VMExitType::Trace {
                                        values: vec![("obj pointer".to_string(), load_to_location)],
                                    },
                                },
                            ));
                            ...
                            initial_ir.push((
                                current_offset,
                                IRInstr::BranchToLabel {
                                    label: *function_start_label,
                                },
                            ));
                            initial_ir.push((
                                current_offset,
                                IRInstr::Label(IRLabel {
                                    name: after_call_label,
                                }),
                            ));
                                ...
                                | CompressedParsedDescriptorType::Ref(_) => {
                                    ...
                                    initial_ir.push((
                                        ...
                                        IRInstr::StoreFPRelative {
                                            from: return_address,
                                            to: layout
                                                .operand_stack_entry(next_byte_code_instr_count, 0),
                                        },
                                    ))
                                }
                            }
                ...
                CompressedInstructionInfo::monitorenter => {
                    ...
                    initial_ir.push((
                        ...
                        IRInstr::VMExit {
                            exit_label,
                            exit_type: VMExitType::MonitorEnter {
                                ref_offset: layout
                                    .operand_stack_entry(current_byte_code_instr_count, 0),
                            },
                        },
                ...
                CompressedInstructionInfo::monitorexit => {
                    ...
                    initial_ir.push((
                        ...
                        IRInstr::VMExit {
                            exit_label,
                            exit_type: VMExitType::MonitorExit {
                                ref_offset: layout
                                    .operand_stack_entry(current_byte_code_instr_count, 0),
                            },
                        },
        ...
        for (offset, ir_instr) in initial_ir {
            ...
            ir.push((
                ...
                byte_code
                    .iter()
                    .find(|instr| instr.offset == offset.0)
                    .unwrap()
                    .clone()
                    .clone(),
            ));
    ...
    fn gen_istore_n(
        ...
        initial_ir.push((
            ...
            IRInstr::StoreFPRelative {
                from: temp,
                to: layout.local_var_entry(current_byte_code_instr_count, n),
            },
        ));
    }
    ...
    fn gen_aload_n(
        ...
        initial_ir.push((
            current_offset,
            IRInstr::LoadFPRelative {
                from: layout.local_var_entry(current_byte_code_instr_count, n),
                to: temp,
            },
        ));
        initial_ir.push((
    ...
    fn gen_code_athrow(
        ...
        initial_ir.push((
            ...
            IRInstr::VMExit {
                exit_label,
                exit_type: VMExitType::Throw {
                    res: layout.operand_stack_entry(current_byte_code_instr_count, 0),
                },
            },
        ));
    }

    fn gen_code_left_ldc(
        &mut self,
        mut labels: &mut Vec<IRLabel>,
        initial_ir: &mut Vec<(ByteCodeOffset, IRInstr)>,
        layout: &dyn StackframeMemoryLayout,
        ...
        match left_ldc {
            CompressedLdcW::String { str } => {
                ...
                initial_ir.push((
                    ...
                    IRInstr::VMExit {
                        exit_label,
                        exit_type: VMExitType::LoadString {
                            string: str.clone(),
        ...
        initial_ir.push((
            ...
            IRInstr::VMExit {
                ...
                exit_type: VMExitType::PutStatic {
                    ...
                    name: *name,
                    frame_pointer_offset_of_to_put: layout
                        .operand_stack_entry(current_byte_code_instr_count, 0),
                },
            },
        ))
            ...
            Some((method_id, is_native)) => {
                if is_native {
                    ...
                    initial_ir.push((
                        ...
                        IRInstr::StoreFPRelative {
                            from: method_id_register,
                            to: FramePointerOffset(method_id_ptr),
                        },
                    ));
                    let magic_1 = layout.full_frame_size() + 4 * size_of::<*mut c_void>();
                    let magic_1_register = Register(3);
                    ...
                    let temp_arg_register = Register(5);
                    for i in local_vars_start..descriptor.arg_types.len() {
                        let load_from_location =
                            layout.operand_stack_entry(current_byte_code_instr_count, i as u16);
                        let load_to_location =
                            FramePointerOffset(local_vars_start + i * size_of::<jlong>());
                        initial_ir.push((
                            current_offset,
        ...
        match resolver.lookup_type_loaded(&(*class_name).into()) {
            ...
            Some((rc, loader)) => {
                ...
                let _todo_manual_allocation_closure = |assembler: &mut CodeAssembler| {
                    ...
                    assembler
                        .add(current_bitmap_ptr, start_index_div_size)
                        .unwrap();
    ...
    pub fn ir_to_native(
        ...
        let mut iced_labels = ir_labels
            .into_iter()
            .map(|label| (label.name, assembler.create_label()))
            .collect::<HashMap<_, _>>();
        let mut label_instruction_offsets: Vec<(LabelName, u32)> = vec![];
        let mut instruction_index_to_bytecode_offset_start: HashMap<
            u32,
            (ByteCodeOffset, CInstruction),
        > = HashMap::new();
        for (bytecode_offset, ir_instr, cinstruction) in ir {
            let cinstruction: CInstruction = cinstruction;
            instruction_index_to_bytecode_offset_start.insert(
                assembler.instructions().len() as u32,
                (bytecode_offset, cinstruction),
            );
            match ir_instr {
                ...
                IRInstr::StoreFPRelative { from, to } => {
                    assembler
                        .mov(qword_ptr(rbp + to.0), from.to_native_64())
                        .unwrap();
                }
                IRInstr::Load { from_address, to } => {
                    assembler
                        .mov(to.to_native_64(), from_address.to_native_64() + 0)
                        .unwrap();
                }
                IRInstr::Store { from, to_address } => {
                    assembler
                        .mov(qword_ptr(to_address.to_native_64()), from.to_native_64())
                        .unwrap();
                }
                IRInstr::Add { res, a } => {
                ...
                IRInstr::Sub { res, to_subtract } => {
                    assembler
                        .sub(res.to_native_64(), to_subtract.to_native_64())
                        .unwrap();
                }
                IRInstr::Div { .. } => todo!(),
                IRInstr::Mod { .. } => todo!(),
                ...
                IRInstr::VMExit {
                    ...
                    let native_stack_pointer = (offset_of!(JitCodeContext, native_saved)
                        + offset_of!(SavedRegisters, stack_pointer))
                        as i64;
                    let native_frame_pointer = (offset_of!(JitCodeContext, native_saved)
                        + offset_of!(SavedRegisters, frame_pointer))
                        as i64;
                    let native_instruction_pointer = (offset_of!(JitCodeContext, native_saved)
                        + offset_of!(SavedRegisters, instruction_pointer))
                        as i64;
                    let java_stack_pointer = (offset_of!(JitCodeContext, java_saved)
                        + offset_of!(SavedRegisters, stack_pointer))
                        as i64;
                    let java_frame_pointer = (offset_of!(JitCodeContext, java_saved)
                        + offset_of!(SavedRegisters, frame_pointer))
                        as i64;
                    ...
                    // call_to_old
                    assembler
                        .call(qword_ptr(r15 + native_instruction_pointer))
                        .unwrap();
                    exits.insert(exit_label, exit_type);
                    label_instruction_offsets
                        .push((exit_label, assembler.instructions().len() as u32));
                ...
                IRInstr::Label(label) => {
                    ...
                    label_instruction_offsets
                        .push((label.name, assembler.instructions().len() as u32));
                ...
                IRInstr::Return {
                    ...
                    assembler
                        .lea(address_base, rbp + size_of::<*mut c_void>() * 2)
                        .unwrap(); // skip saved rip and saved rbp
                    ...
                    assembler
                        .lea(address, address_base + index * size_of::<*mut c_void>())
                        .unwrap();
                    ...
                    assembler
                        .mov(rbp, rbp + offset_of!(FrameHeader, prev_rpb))
                        .unwrap();
                ...
                IRInstr::LoadLabel { label, to } => {
                    ...
                    assembler
                        .lea(to.to_native_64(), dword_ptr(iced_label))
                        .unwrap();
                }
                IRInstr::LoadRBP { to } => {
        ...
        for (i, instruction) in assembler.instructions().iter().enumerate() {
            ...
            let instruction_info_as_string =
                &match instruction_index_to_bytecode_offset_start.get(&(i as u32)) {
        ...
        for (i, native_offset) in result.new_instruction_offsets.iter().enumerate() {
            ...
            let current_instruction_address =
                unsafe { base_address.offset(*native_offset as isize) };
            loop {
            ...
            if let Some((new_byte_code_offset, cinstr)) =
            instruction_index_to_bytecode_offset_start.get(&(i as u32))
            {
                assert!(
                    ((current_byte_code_start_address.unwrap()) as u64)
                        < ((current_instruction_address) as u64)
                );
                bytecode_offset_to_address.insert_range(
                    current_byte_code_start_address.unwrap()..current_instruction_address,
                    (
                        current_byte_code_offset.unwrap().0,
                        current_byte_code_offset.unwrap().1,
                        cinstr.clone(),
                    ),
                );
    ...
    fn add_from_ir(
        &mut self,
        method_log_info: String,
        current_code_id: CompiledCodeID,
        ir: ToIR,
    ) -> *mut c_void {
        let ToNative {
            code,
        ...
        self.function_starts
            .insert(current_code_id, function_start_label);
        ...
        let bytecode_offset_to_address: BiRangeMap<*mut c_void, (_, _, _)> =
            bytecode_offset_to_address;
        for (address_range, offset) in bytecode_offset_to_address {
            self.address_to_byte_code_index
                .entry(current_code_id)
                .or_insert(BiRangeMap::new())
                .insert_range(address_range.clone(), offset.0);
            self.address_to_byte_code_offset
                .entry(current_code_id)
                .or_insert(BiRangeMap::new())
                .insert_range(address_range.clone(), offset.1);
            self.address_to_byte_code_compressed_code
                .entry(current_code_id)
                .or_insert(BiRangeMap::new())
                .insert_range(address_range, offset.2);
        }
        unsafe { copy_nonoverlapping(code.as_ptr(), install_at as *mut u8, code.len()) }
        unsafe {
            self.function_addresses.insert_range(
                install_at..(install_at.offset(code.len() as isize)),
                current_code_id,
            );
        }
        install_at
    ...
    pub fn recompile_method_and_restart(
        ...
        let instruct_pointer = int_state
            .get_java_stack()
            .saved_registers()
            .instruction_pointer;
        assert_eq!(instruct_pointer, old_java_ip);
        let compiled_code_id = *jit_state
            .borrow()
            .function_addresses
            .get(&instruct_pointer)
            .unwrap();
        let temp = jit_state.borrow();
        let compiled_code = temp
            .address_to_byte_code_offset
            .get(&compiled_code_id)
            .unwrap();
        ...
        drop(temp);
        let new_base_address = jit_state.borrow_mut().add_function(
            code,
            methodid,
            MethodResolver {
                jvm,
        ...
        let new_code_id = *jit_state
            .borrow()
            .function_addresses
            .get(&new_base_address)
            .unwrap();
        let start_byte_code_addresses = jit_state
            .borrow()
            .address_to_byte_code_offset
            .get(&new_code_id)
            .unwrap()
            .get_reverse(&return_to_byte_code_offset)
            .unwrap()
            .clone();
    ...
    fn runtime_type_info(memory_region: &MutexGuard<MemoryRegions>) -> RuntimeTypeInfo {
        unsafe {
            RuntimeTypeInfo {
                ...
                extra_large_region_index_to_region_data: memory_region
                    .extra_large_region_types
                    .as_ptr(),
    ...
    unsafe fn resume_method(
        jit_state: &RefCell<JITedCodeState>,
        mut target_ip: *mut c_void,
        jvm: &'gc_life JVMState<'gc_life>,
        int_state: &mut InterpreterStateGuard<'gc_life, '_>,
        methodid: MethodId,
        compiled_id: CompiledCodeID,
    ) -> Result<Option<JavaValue<'gc_life>>, WasException> {
        loop {
            ...
            let SavedRegisters {
                stack_pointer,
                frame_pointer,
                instruction_pointer: as_ptr,
                status_register,
            } = java_stack.handle_vm_entry();
            ...
            eprint!(
                "going in sp:{:?} fp:{:?} ip: {:?}",
                jit_code_context.java_saved.stack_pointer,
                jit_code_context.java_saved.frame_pointer,
                jit_code_context.java_saved.instruction_pointer
            );
            let mut jit_state_ = jit_state.borrow();
            let compiled_code_id = jit_state_.function_addresses.get(&target_ip).unwrap();
            let method_id = jit_state_
                .method_id_to_code
                .get_by_right(compiled_code_id)
                .unwrap();
            let (rc, method_i) = jvm
                .method_table
                .read()
            ...
            eprintln!(
                "@ {:?}:{:?}",
                view.name().unwrap_object_name().0.to_str(string_pool),
                method_view.name().0.to_str(string_pool)
            );
            drop(jit_state_);
            ...
            eprintln!(
                "going out sp:{:?} fp:{:?} ip:{:?} {} {} {:?}",
                java_stack.saved_registers.unwrap().stack_pointer,
                java_stack.saved_registers.unwrap().frame_pointer,
                java_stack.saved_registers.unwrap().instruction_pointer,
                class_name_str,
                method_name_str,
                exit_type
            );
            target_ip = match JITedCodeState::handle_exit(
                jit_state,
                exit_type,
                jvm,
                int_state,
    ...
    #[allow(named_asm_labels)]
    pub unsafe fn run_method(
        jitstate: &RefCell<JITedCodeState>,
        jvm: &'gc_life JVMState<'gc_life>,
        int_state: &mut InterpreterStateGuard<'gc_life, '_>,
        methodid: MethodId,
        compiled_id: CompiledCodeID,
    ) -> Result<Option<JavaValue<'gc_life>>, WasException> {
        let target_ip = jitstate
            .borrow()
            .function_addresses
            .get_reverse(&compiled_id)
            .unwrap()
            .start;
    ...
    fn handle_exit(
        ...
        match exit_type {
            VMExitType::ResolveInvokeStatic {
                method_name,
                desc,
                target_class,
            } => {
                ...
                let inited_class =
                    check_initing_or_inited_class(jvm, int_state, target_class).unwrap();
                int_state.get_java_stack().saved_registers = save;
                let method_view = inited_class
                    .unwrap_class_class()
                    .class_view
                    .lookup_method(method_name, &desc)
                    .unwrap();
                let to_call_function_method_id = jvm
                    .method_table
                    .write()
                    .unwrap()
                    .get_method_id(inited_class.clone(), method_view.method_i());
                if method_view.is_native() {
                    ...
                    jitstate.borrow_mut().add_function(
                        method_view.code_attribute().unwrap(),
                        to_call_function_method_id,
                        MethodResolver {
                            jvm,
                            loader: int_state.current_loader(),
                        },
                    );
                    let (current_function_rc, current_function_method_i) = jvm
                        .method_table
                        .read()
                        .unwrap()
                        .try_lookup(methodid)
                        .unwrap();
                    let method_view = current_function_rc
                        .unwrap_class_class()
                        .class_view
                        .method_view_i(current_function_method_i);
                    let code = method_view.code_attribute().unwrap();
                    Self::recompile_method_and_restart(
                        jitstate,
                        methodid,
                        jvm,
                        int_state,
            ...
            VMExitType::RunNativeStatic {
                ...
                let address_to_bytecode_for_this_method =
                    jit_state.address_to_byte_code_offset.get(&code_id).unwrap();
                let bytecode_offset = address_to_bytecode_for_this_method
                    .get(&old_java_ip)
                    .unwrap();
                let restart_bytecode_offset = ByteCodeOffset(bytecode_offset.0 + 3); // call static is 3 bytes
                let restart_address = address_to_bytecode_for_this_method
                    .get_reverse(&restart_bytecode_offset)
                    .unwrap()
                    .start;
                Some(restart_address)
            }
            VMExitType::PutStatic {
                target_type,
                target_class,
                name,
                frame_pointer_offset_of_to_put,
            } => {
                let save = int_state.get_java_stack().saved_registers;
                let target_class_rc =
                    check_initing_or_inited_class(jvm, int_state, target_class).unwrap();
                ...
                target_class_rc.static_vars().insert(
                    name,
                    int_state.raw_read_at_frame_pointer_offset(
                        frame_pointer_offset_of_to_put,
                        target_type.to_runtime_type().unwrap(),
                    ),
                );
                //todo dup
                let jitstate_borrow = jitstate.borrow();
                let code_id = jitstate_borrow
                    .method_id_to_code
                    .get_by_left(&methodid)
                    .unwrap();
                let address_to_bytecode_for_this_method = jitstate_borrow
                    .address_to_byte_code_offset
                    .get(&code_id)
                    .unwrap();
                let bytecode_offset = address_to_bytecode_for_this_method
            ...
            VMExitType::AllocateVariableSizeArrayANewArray {
                target_type_sub_type,
                len_offset,
                res_write_offset,
            } => {
                ...
                let before_stack = int_state
                    .get_java_stack()
                    .saved_registers
                    .unwrap()
                    .stack_pointer;
                let save = int_state.get_java_stack().saved_registers;
                let inited_target_type_rc =
                    check_initing_or_inited_class(jvm, int_state, target_type_sub_type).unwrap();
                int_state.get_java_stack().saved_registers = save;
                assert_eq!(
                    before_stack,
                    int_state
                ...
                // dbg!(int_state.get_java_stack().saved_registers.unwrap().frame_pointer);
                // dbg!(int_state.get_java_stack().saved_registers.unwrap().stack_pointer);
                let array_len = int_state
                    .raw_read_at_frame_pointer_offset(len_offset, RuntimeType::IntType)
                    .unwrap_int() as usize;
                let allocated_object_type = runtime_class_to_allocated_object_type(
                    &inited_target_type_rc,
                ...
                let to_write = jvalue {
                    l: allocation.as_ptr() as jobject,
                };
            ...
            VMExitType::InitClass { target_class } => {
                ...
                let compiled_code = jit_state
                    .address_to_byte_code_offset
                    .get(&compiled_code_id)
                    .unwrap();
                // problem here is that a function call overwrites the old old ip
                let return_to_byte_code_offset = *compiled_code.get(&instruct_pointer).unwrap();
                let new_base_address = jit_state.add_function(
                    code,
                    methodid,
                    MethodResolver {
                        jvm,
            ...
            VMExitType::Allocate {
                ...
                let save = int_state.get_java_stack().saved_registers;
                let rc =
                    check_loaded_class_force_loader(jvm, int_state, &ptypeview, loader).unwrap();
                int_state.get_java_stack().saved_registers = save;
                ...
                int_state.raw_write_at_frame_pointer_offset(
                    res,
                    jvalue {
                        l: allocated.raw_ptr_usize() as jobject,
                    },
                );
                let jitstate_borrow = jitstate.borrow();
                let code_id = jitstate_borrow
            ...
            VMExitType::LoadClass {
                ...
                let to_write = jvalue {
                    l: class.object().raw_ptr_usize() as jobject,
                };
            ...
            VMExitType::Trace { values } => {
                ...
                let address_to_bytecode_for_this_method = jitstate_borrow
                    ...
                    .get(&code_id)
                    .unwrap();
                let next_java_ip: *mut c_void = address_to_bytecode_for_this_method
                    .keys()
                    .map(|key| key.start)
                    .sorted()
                    .filter(|pointer| pointer > &old_java_ip)
                    .next()
                    .unwrap();
                let restart_bytecode_offset = address_to_bytecode_for_this_method
                    .get(&next_java_ip)
                    .unwrap();
                let restart_address = address_to_bytecode_for_this_method
                    .get_reverse(&restart_bytecode_offset)
...
pub fn runtime_class_to_allocated_object_type(
    ref_type: &RuntimeClass,
    loader: LoaderName,
    arr_len: Option<usize>,
    thread_id: JavaThreadId,
) -> AllocatedObjectType {
    match ref_type {
...
impl StackframeMemoryLayout for NaiveStackframeLayout {
    ...
    fn operand_stack_entry(&self, current_count: u16, from_end: u16) -> FramePointerOffset {
        FramePointerOffset(
            size_of::<FrameHeader>()
                + (self.max_locals + self.stack_depth[&current_count] - from_end) as usize
                * size_of::<jlong>(),
        )
    }
    ...
    fn full_frame_size(&self) -> usize {
        size_of::<FrameHeader>()
            + (self.max_locals as usize + self.max_stack as usize + 1) * size_of::<jlong>()
...
pub fn setup_args_from_current_frame(
    jvm: &'gc_life JVMState<'gc_life>,
    int_state: &mut InterpreterStateGuard<'gc_life, '_>,
    desc: &CMethodDescriptor,
    is_virtual: bool,
) -> Vec<JavaValue<'gc_life>> {
    if is_virtual {
        todo!()
    }
