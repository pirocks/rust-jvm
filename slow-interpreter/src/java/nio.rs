pub mod heap_byte_buffer {
    ...
    impl HeapByteBuffer {
        pub fn new(jvm: &JVMState, int_state: &mut InterpreterStateGuard, buf: Vec<jbyte>, off: jint, len: jint) -> Self {
            ...
            let array_object = ArrayObject::new_array(jvm, int_state, elems, PTypeView::ByteType, jvm.thread_state.new_monitor("heap bytebuffer array monitor".to_string()), int_state.current_loader());
