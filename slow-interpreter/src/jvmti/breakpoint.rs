...
pub unsafe extern "C" fn set_breakpoint(
    env: *mut jvmtiEnv,
    method: jmethodID,
    location: jlocation,
) -> jvmtiError {
    ...
    let tracing_guard = jvm
        .config
        .tracing
        .trace_jdwp_function_enter(jvm, "SetBreakpoint");
    ...
    match breakpoint_guard.get_mut(&method_id) {
        None => {
            breakpoint_guard.insert(
                method_id,
                HashSet::from_iter(vec![location as u16].iter().cloned()),
            );
        }
        Some(breakpoints) => {
...
pub unsafe extern "C" fn clear_breakpoint(
    env: *mut jvmtiEnv,
    method: jmethodID,
    location: jlocation,
) -> jvmtiError {
    let jvm = get_state(env);
    let tracig_guard = jvm
        .config
        .tracing
        .trace_jdwp_function_enter(jvm, "ClearBreakpoint");
    let method_id = from_jmethod_id(method);
    jvm.jvmti_state()
        .unwrap()
        .break_points
        ...
        .get_mut(&method_id)
        .unwrap()
        .remove(&(location as u16));
    jvm.config
        .tracing
        .trace_jdwp_function_exit(tracig_guard, jvmtiError_JVMTI_ERROR_NONE)
}
