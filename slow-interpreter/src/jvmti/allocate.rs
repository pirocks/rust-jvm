...
use jvmti_jni_bindings::{
    jlong, jvmtiEnv, jvmtiError, jvmtiError_JVMTI_ERROR_ILLEGAL_ARGUMENT,
    jvmtiError_JVMTI_ERROR_MUST_POSSESS_CAPABILITY, jvmtiError_JVMTI_ERROR_NONE,
...
pub unsafe extern "C" fn allocate(
    ...
    let tracing_guard = jvm
        .config
        .tracing
        .trace_jdwp_function_enter(jvm, "Allocate");
    if size < 0 {
    ...
    *mem_ptr = jvm
        ...
        .allocate_malloc(size as usize) as *mut ::std::os::raw::c_uchar;
...
pub unsafe extern "C" fn deallocate(
    ...
    let tracing_guard = jvm
        .config
        .tracing
        .trace_jdwp_function_enter(jvm, "Deallocate");
    jvm.native
        .native_interface_allocations
...
pub unsafe extern "C" fn dispose_environment(env: *mut jvmtiEnv) -> jvmtiError {
    ...
    let tracing_guard = jvm
        .config
        .tracing
        .trace_jdwp_function_enter(jvm, "DisposeEnvironment");
    jvm.config.tracing.trace_jdwp_function_exit(
        tracing_guard,
