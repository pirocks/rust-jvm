...
pub unsafe extern "C" fn get_version_number(
    env: *mut jvmtiEnv,
    version_ptr: *mut jint,
) -> jvmtiError {
    let jvm = get_state(env);
    let tracing_guard = jvm
        .config
        .tracing
        .trace_jdwp_function_enter(jvm, "GetVersionNumber");
    null_check!(version_ptr);
    let version = (JVMTI_VERSION_INTERFACE_JVMTI
        | (JVMTI_VERSION_MASK_MAJOR & (JVMTI_INTERFACE_MAJOR_VERSION << 16))
        | (JVMTI_VERSION_MASK_MINOR & (JVMTI_INTERFACE_MINOR_VERSION << 8)))
        as u32;
