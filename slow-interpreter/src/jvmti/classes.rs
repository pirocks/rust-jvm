...
use jvmti_jni_bindings::{
    jclass, jint, jmethodID, jobject, JVMTI_CLASS_STATUS_ARRAY, JVMTI_CLASS_STATUS_INITIALIZED,
    JVMTI_CLASS_STATUS_PREPARED, JVMTI_CLASS_STATUS_PRIMITIVE,
    JVMTI_CLASS_STATUS_VERIFIED, jvmtiEnv, jvmtiError,
    jvmtiError_JVMTI_ERROR_ABSENT_INFORMATION, jvmtiError_JVMTI_ERROR_INVALID_CLASS, jvmtiError_JVMTI_ERROR_NONE,
};
...
pub unsafe extern "C" fn get_source_file_name(
    ...
    let tracing_guard = jvm
        .config
        .tracing
        .trace_jdwp_function_enter(jvm, "GetSourceFileName");
    ...
    if let Some(file) = sourcefile {
        ...
        jvm.config
            .tracing
            .trace_jdwp_function_exit(tracing_guard, jvmtiError_JVMTI_ERROR_ABSENT_INFORMATION)
    }
}
...
pub unsafe extern "C" fn get_implemented_interfaces(
    ...
    let tracing_guard = jvm
        .config
        .tracing
        .trace_jdwp_function_enter(jvm, "GetImplementedInterfaces");
    let class_obj = from_jclass(jvm, klass);
    let runtime_class = class_obj.as_runtime_class(jvm);
    let class_view = runtime_class.view();
...
pub unsafe extern "C" fn get_class_status(
    env: *mut jvmtiEnv,
    klass: jclass,
    status_ptr: *mut jint,
) -> jvmtiError {
    ...
    let tracing_guard = jvm
        .config
        .tracing
        .trace_jdwp_function_enter(jvm, "GetClassStatus");
    let class = from_object(jvm, klass as jobject).unwrap(); //todo handle null
    let res = {
        let type_ = &JavaValue::Object(class.into())
            .cast_class()
            .unwrap()
            .as_type(jvm);
        let mut status = 0;
...
pub unsafe extern "C" fn get_loaded_classes<'gc_life>(
    env: *mut jvmtiEnv,
    ...
    let tracing_guard = jvm
        .config
        .tracing
        .trace_jdwp_function_enter(jvm, "GetLoadedClasses");
    let mut res_vec = vec![];
    ...
    for (_loader, ptype) in collected {
        ...
        res_vec.push(new_local_ref_public(
            class_object.unwrap().into(),
            int_state,
        ))
    }
...
pub unsafe extern "C" fn get_class_signature(
    env: *mut jvmtiEnv,
    klass: jclass,
    ...
    let tracing_guard = jvm
        .config
        .tracing
        .trace_jdwp_function_enter(jvm, "GetClassSignature");
    let notnull_class = from_object(jvm, klass as jobject).unwrap(); //todo handle npe
    let class_object_ptype = JavaValue::Object(todo!() /*notnull_class.into()*/)
        .cast_class()
        .unwrap()
        .as_type(jvm);
    ...
    if !signature_ptr.is_null() {
        let jvm_repr =
            CString::new(PTypeView::from_compressed(&type_, &jvm.string_pool).jvm_representation())
                .unwrap();
        let jvm_repr_ptr = jvm_repr.into_raw();
        let allocated_jvm_repr =
            libc::malloc(libc::strlen(jvm_repr_ptr) + 1) as *mut ::std::os::raw::c_char;
        signature_ptr.write(allocated_jvm_repr);
        libc::strcpy(allocated_jvm_repr, jvm_repr_ptr);
    }
    if !generic_ptr.is_null() {
        let java_repr = CString::new(
            PTypeView::from_compressed(&type_, &jvm.string_pool).java_source_representation(),
        )
            .unwrap();
        let java_repr_ptr = java_repr.into_raw();
        let allocated_java_repr =
            libc::malloc(libc::strlen(java_repr_ptr) + 1) as *mut ::std::os::raw::c_char;
...
pub unsafe extern "C" fn get_class_methods(
    env: *mut jvmtiEnv,
    klass: jclass,
    ...
    let tracing_guard = jvm
        .config
        .tracing
        .trace_jdwp_function_enter(jvm, "GetClassMethods");
    ...
    let res = loaded_class
        ...
        .map(|mv| {
            let method_id = jvm
                ...
                .write()
                .unwrap()
                .get_method_id(loaded_class.clone(), mv.method_i() as u16);
            method_id as jmethodID
        })
        .collect::<Vec<_>>();
    jvm.native
        .native_interface_allocations
        .allocate_and_write_vec(res, method_count_ptr, methods_ptr);
    jvm.config
        .tracing
        .trace_jdwp_function_exit(tracing_guard, jvmtiError_JVMTI_ERROR_NONE)
}

pub unsafe extern "C" fn get_class_loader(
    env: *mut jvmtiEnv,
    klass: jclass,
