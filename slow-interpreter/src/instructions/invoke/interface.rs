...
pub fn invoke_interface<'l, 'gc_life>(
    ...
    let this_pointer_jv: JavaValue<'gc_life> = operand_stack_ref.get(
        operand_stack_len - (desc_len + 1)/*count.get()*/ as u16,
        RuntimeType::object(),
    );
    ...
    let (target_method_i, final_target_class) = find_target_method(
        jvm,
        int_state,
    ...
    let _ = invoke_virtual_method_i(
        jvm,
        int_state,
        expected_descriptor,
        final_target_class.clone(),
        &final_target_class.view().method_view_i(target_method_i),
        todo!(),
    );
}