...
pub fn run_native_method(
    ...
    let native_call_frame = int_state.push_frame(
        StackEntry::new_native_frame(jvm, class.clone(), method_i as u16, args.clone()),
        jvm,
    );
    assert!(int_state.current_frame().is_native());
    let monitor = monitor_for_function(
        jvm,
        int_state,
        &method,
        method.access_flags() & JVM_ACC_SYNCHRONIZED as u16 > 0,
    );
    ...
    let result = if jvm
        ...
        .read()
        .unwrap()
        .contains_key(&ByAddress(class.clone()))
        && jvm
        .native_libaries
        .registered_natives
        .read()
        .unwrap()
        .get(&ByAddress(class.clone()))
        .unwrap()
        .read()
        .unwrap()
        .contains_key(&(method_i as u16))
    {
        //todo dup
        let res_fn = {
            let reg_natives = jvm.native_libaries.registered_natives.read().unwrap();
            let reg_natives_for_class = reg_natives
                .get(&ByAddress(class.clone()))
                .unwrap()
                .read()
                .unwrap();
        ...
        match match call(
            ...
            Some(r) => r,
            None => {
                match special_call_overrides(
                    jvm,
                    int_state,
                    &class.view().method_view_i(method_i),
                    args,
                ) {
                    Ok(res) => res,
                    Err(_) => None,
...
fn special_call_overrides(
    ...
    Ok(
        if &mangled == "Java_java_lang_invoke_MethodHandleNatives_registerNatives" {
            ...
            Java_java_lang_invoke_MethodHandleNatives_objectFieldOffset(jvm, int_state, args)?
                .into()
            ...
            Some(
                HeapByteBuffer::new(jvm, int_state, vec![0, 0, 0, 0, 0, 0, 0, 0], 0, 8)?
                    .java_value(),
