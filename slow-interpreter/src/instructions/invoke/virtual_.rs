 ...
 */
pub fn invoke_virtual_instruction(
...
fn invoke_virtual_method_i_impl<'gc_life>(
    ...
    if target_method.is_signature_polymorphic() {
        ...
        let temp_value = op_stack.get(
            (op_stack.len() - (expected_descriptor.arg_types.len() as u16 + 1)) as u16,
        ...
        if target_method.name() == MethodName::method_invoke()
            || target_method.name() == MethodName::method_invokeBasic()
            || target_method.name() == MethodName::method_invokeExact()
        {
            //todo do conversion.
...
pub fn call_vmentry(
    jvm: &'gc_life JVMState<'gc_life>,
    interpreter_state: &'_ mut InterpreterStateGuard<'gc_life, '_>,
    vmentry: MemberName<'gc_life>,
) -> Result<JavaValue<'gc_life>, WasException> {
    assert_eq!(
        vmentry.clone().java_value().to_type(),
...
pub fn setup_virtual_args<'gc_life>(
...
pub fn setup_virtual_args2<'gc_life>(
    int_state: &'_ mut InterpreterStateGuard<'gc_life, '_>,
    expected_descriptor: &CMethodDescriptor,
    args: &mut Vec<JavaValue<'gc_life>>,
    max_locals: u16,
...
pub fn invoke_virtual(
    ...
    let this_pointer = {
        ...
        &operand_stack.get(
            (operand_stack.len() as usize - md.arg_types.len() - 1) as u16,
            RuntimeType::object(),
        )
    };
