...
pub fn pop2(
    ...
    match value1.to_type() {
        ...
        _ => {
            if let JavaValue::Long(_) | JavaValue::Double(_) =
            current_frame.pop(Some(RuntimeType::IntType))
            {
                panic!()
            };
        }
...
pub fn pop(jvm: &'gc_life JVMState<'gc_life>, mut current_frame: StackEntryMut<'gc_life, 'l>) {
    current_frame.pop(Some(RuntimeType::LongType));
}