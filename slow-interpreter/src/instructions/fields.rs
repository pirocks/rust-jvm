...
pub fn putstatic(
    ...
    let field_value = stack
        .pop(Some(field_descriptor.0.to_runtime_type().unwrap()))
        .unwrap();
    target_classfile
        .static_vars()
...
pub fn putfield(
    ...
    let val = stack
        .pop(Some(field_type.to_runtime_type().unwrap()))
        .unwrap();
...
pub fn get_static(
    ...
    field_class_name: CClassName,
    field_name: FieldName,
    _field_descriptor: &CFieldDescriptor,
) {
...
fn get_static_impl(
    ...
    let target_classfile =
        check_initing_or_inited_class(jvm, int_state, field_class_name.clone().into())?;
    //todo handle interfaces in setting as well
    for interfaces in target_classfile.view().interfaces() {
        let interface_lookup_res = get_static_impl(
            jvm,
            int_state,
    ...
    match object_ref {
        JavaValue::Object(o) => {
            let res =
                o.unwrap()
                    .unwrap_normal_object()
                    .get_var(jvm, target_class_pointer, field_name);
