...
pub fn anewarray(
    ...
    let len = match int_state
        .current_frame_mut()
        .pop(Some(RuntimeType::IntType))
    {
        JavaValue::Int(i) => i,
        _ => panic!(),
    };
...
pub fn newarray(
    ...
    let new_array = match JavaValue::new_vec(
        jvm,
        int_state,
        count as usize,
        default_value(type_.clone()),
        type_,
    ) {
        Ok(arr) => arr,
        ...
        current = JavaValue::Object(
            jvm.allocate_object(Object::Array(
                match ArrayObject::new_array(
                    ...
                    next_type.clone(),
                    jvm.thread_state
                        .new_monitor("monitor for a multi dimensional array".to_string()),
                ) {
                    Ok(arr) => arr,
                    Err(WasException {}) => return,
                },
