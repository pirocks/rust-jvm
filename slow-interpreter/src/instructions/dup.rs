...
pub fn dup_x2(jvm: &'_ JVMState<'gc_life>, method_id: MethodId, mut current_frame: StackEntryMut<'gc_life>) {
    ...
    let value1 = current_frame.pop(jvm, PTypeView::LongType);//in principle type doesn't matter
    ...
    match value2_vtype {
        ...
        _ => {
            let value3 = current_frame.pop(jvm, PTypeView::LongType);//in principle type doesn't matter todo pass it anyway
            current_frame.push(jvm, value1.clone());
...
pub fn dup2(jvm: &'_ JVMState<'gc_life>, method_id: MethodId, mut current_frame: StackEntryMut<'gc_life>) {
    ...
    let value1 = current_frame.pop(jvm, PTypeView::LongType);//in principle type doesn't matter todo pass it anyway
    match value1_vtype {
        VType::LongType | VType::DoubleType => {
            current_frame.push(jvm, value1.clone());
            current_frame.push(jvm, value1);
