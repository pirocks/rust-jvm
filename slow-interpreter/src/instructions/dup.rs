...
pub fn dup_x2(jvm: &'_ JVMState<'gc_life>, method_id: MethodId, mut current_frame: StackEntryMut) {
...
pub fn dup2(jvm: &'_ JVMState<'gc_life>, method_id: MethodId, mut current_frame: StackEntryMut) {
    let current_pc = current_frame.to_ref().pc();
    let stack_frames = &jvm.function_frame_type_data.read().unwrap()[&method_id];
    let Frame { stack_map: OperandStack { data }, .. } = &stack_frames[&current_pc];
...
pub fn dup2_x1(jvm: &'_ JVMState<'gc_life>, method_id: MethodId, mut current_frame: StackEntryMut) {
    let current_pc = current_frame.to_ref().pc();
    let stack_frames = &jvm.function_frame_type_data.read().unwrap()[&method_id];
    let Frame { stack_map: OperandStack { data }, .. } = &stack_frames[&current_pc];
