pub mod reflection {
    ...
    impl<'gc_life> Reflection<'gc_life> {
        pub fn is_same_class_package(
            jvm: &'gc_life JVMState<'gc_life>,
            int_state: &'_ mut InterpreterStateGuard<'gc_life, 'l>,
            class1: JClass<'gc_life>,
            class2: JClass<'gc_life>,
        ) -> Result<jboolean, WasException> {
            let reflection =
                check_initing_or_inited_class(jvm, int_state, CClassName::reflection().into())?;
            ...
            run_static_or_virtual(
                ...
                int_state,
                &reflection,
                MethodName::method_isSameClassPackage(),
                &desc,
                todo!(),
            )?;
            Ok(int_state
