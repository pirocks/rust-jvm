...
impl Debug for RuntimeClassClass<'_> {
    fn fmt(&self, f: &mut Formatter<'_>) -> Result<(), Error> {
        write!(f, "{:?}:{:?}", self.class_view.name(), self.static_vars)
    }
}

pub fn prepare_class<'l, 'k : 'l, 'gc_life>(jvm: &'gc_life JVMState<'gc_life>, int_state: &'k mut InterpreterStateGuard<'l, 'gc_life>, classfile: Arc<dyn ClassView>, res: &mut HashMap<String, JavaValue<'gc_life>>) {
...
pub fn initialize_class<'gc_life, 'l>(
    runtime_class: Arc<RuntimeClass<'gc_life>>,
    jvm: &'gc_life JVMState<'gc_life>,
    interpreter_state: &'l mut InterpreterStateGuard<'l, 'gc_life>,
