...
pub unsafe extern "C" fn get_system_property(env: *mut jvmtiEnv, property: *const ::std::os::raw::c_char, value_ptr: *mut *mut ::std::os::raw::c_char) -> jvmtiError {
    let jvm = get_state(env);
    let tracing_guard = jvm.config.tracing.trace_jdwp_function_enter(jvm, "GetSystemProperty");
    null_check!(property);
    ...
    jvm.config.tracing.trace_jdwp_function_exit(tracing_guard, jvmtiError_JVMTI_ERROR_NOT_AVAILABLE)
}
