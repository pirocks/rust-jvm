...
pub fn mangle(pool: &CompressedClassfileStringPool, method: &MethodView) -> String {
    ...
    let class_name = class_view
        ...
        .replace(".", "/");
    let multiple_same_name_methods = class_view
    ...
    if multiple_same_name_methods {
        ...
        let extracted_descriptor = rg
            .captures(descriptor_str.to_str(pool).as_str())
            .unwrap()
            .get(1)
            .unwrap()
            .as_str()
        ...
        format!(
            "Java_{}_{}__{}",
            escape(&class_name),
            escape(&method_name.0.to_str(pool)),
            escape(&extracted_descriptor)
        )
    } else {
        format!(
            "Java_{}_{}",
