...
//GetFieldID
pub fn get_interface(state: &JVMState) -> &'static JNINativeInterface_ {
    {
        let mut refcell = JNI_Interface.with(|| { RefCell::new(None)/*todo how do I handle this duplication*/ }).borrow_mut();
        match refcell.deref() {
            None => {
                let new = JNINativeInterface_ {
    ...
    JNI_Interface.with(|| { panic!() }).borrow().as_ref().unwrap()
}
