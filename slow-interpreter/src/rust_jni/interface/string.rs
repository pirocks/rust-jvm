...
pub unsafe extern "C" fn get_string_utfchars(
    env: *mut JNIEnv,
    str: jstring,
    is_copy: *mut jboolean,
) -> *const c_char {
    get_rust_str(env, str, |rust_str| {
        let mut buf = JVMString::from_regular_string(rust_str.as_str())
            .buf
            .clone();
        ...
        jvm.native
            ...
            .allocate_and_write_vec(buf, null_mut(), &mut res as *mut *mut u8);
        if !is_copy.is_null() {
...
pub unsafe extern "C" fn release_string_chars(
    env: *mut JNIEnv,
    _str: jstring,
    chars: *const jchar,
) {
    ...
    jvm.native
        .native_interface_allocations
        .free(chars as *mut c_void);
}

pub unsafe extern "C" fn new_string_utf(
    env: *mut JNIEnv,
    ...
    new_local_ref_public(
        match JString::from_rust(
            jvm,
            int_state,
            Wtf8Buf::from_string(str.to_str().unwrap().to_string()),
        ) {
            Ok(jstring) => jstring,
...
pub unsafe fn new_string_with_len(
    env: *mut JNIEnv,
    utf: *const ::std::os::raw::c_char,
    len: usize,
) -> jstring {
...
pub unsafe fn intern_impl_unsafe(
    jvm: &'gc_life JVMState<'gc_life>,
    int_state: &'_ mut InterpreterStateGuard<'gc_life, 'l>,
    str_unsafe: jstring,
) -> Result<jstring, WasException> {
...
pub fn intern_safe<'gc_life>(
    jvm: &'gc_life JVMState<'gc_life>,
    str_obj: GcManagedObject<'gc_life>,
) -> JString<'gc_life> {
    let char_array_ptr = match str_obj
        .clone()
        ...
        .unwrap_object()
    {
        None => {
            ...
            return JavaValue::Object(todo!() /*str_obj.into()*/)
                .cast_string()
                .unwrap(); //fallback to not interning weird strings like this. not sure if compatible with hotspot but idk what else to do. perhaps throwing an exception would be better idk?
        }
...
pub unsafe extern "C" fn get_string_utfregion(
    env: *mut JNIEnv,
    str: jstring,
    start: jsize,
    len: jsize,
...
unsafe fn get_rust_str<T: ExceptionReturn>(
    env: *mut JNIEnv,
    str: jobject,
    and_then: impl Fn(String) -> T,
) -> T {
    let jvm = get_state(env);
    let int_state = get_interpreter_state(env);
    let str_obj = match from_object(jvm, str) {
    ...
    let rust_str = JavaValue::Object(str_obj.into())
        .cast_string()
        .unwrap()
        .to_rust_string(jvm);
    and_then(rust_str)
...
pub unsafe extern "C" fn new_string(
    env: *mut JNIEnv,
    unicode: *const jchar,
    len: jsize,
) -> jstring {
...
pub unsafe extern "C" fn get_string_region(
    env: *mut JNIEnv,
    str: jstring,
    start: jsize,
    len: jsize,
...
pub unsafe extern "C" fn release_string_utfchars(
    env: *mut JNIEnv,
    _str: jstring,
    chars: *const c_char,
) {
