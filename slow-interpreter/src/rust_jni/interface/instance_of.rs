...
pub unsafe extern "C" fn is_instance_of(env: *mut JNIEnv, obj: jobject, clazz: jclass) -> jboolean {
    ...
    let type_view = JavaValue::Object(class_object).to_new().cast_class().expect("todo").as_type(jvm);
    ...
    (int_state.pop_current_operand_stack(RuntimeType::IntType.into()).unwrap_int() != 0) as jboolean
}