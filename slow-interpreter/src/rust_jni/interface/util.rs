...
pub unsafe fn runtime_class_from_object(cls: jclass, state: & JVMState, frame: &Rc<StackEntry>) -> Option<Arc<RuntimeClass>> {
...
pub fn class_object_to_runtime_class(obj: &NormalObject, state: & JVMState, frame: &Rc<StackEntry>) -> Option<Arc<RuntimeClass>> {
    ...
    match obj.class_object_to_ptype().unwrap_ref_type() {
        ReferenceTypeView::Class(class_name) => {
            check_inited_class(state, &class_name, frame.clone().into(), frame.class_pointer.loader.clone()).into()//todo a better way?
        }
