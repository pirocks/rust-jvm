...
impl<'gc> SafePoint<'gc> {
    pub fn check<'l>(&self, jvm: &'gc JVMState<'gc>, int_state: &mut impl HasJavaStack<'gc>) -> Result<(), WasException<'gc>> {
        let guard = self.state.lock().unwrap();
...
impl Monitor2 {
    ...
    pub fn lock<'l, 'gc>(&self, jvm: &'gc JVMState<'gc>, int_state: &mut impl HasJavaStack<'gc>) -> Result<(), WasException<'gc>> {
    ...
    pub fn unlock<'gc, 'l>(&self, jvm: &'gc JVMState<'gc>, int_state: &mut impl HasJavaStack<'gc>) -> Result<(), WasException<'gc>> {
        let mut guard = self.monitor2_priv.write().unwrap();
        let current_thread = jvm.thread_state.get_current_thread();
    ...
    pub fn wait<'gc, 'k>(&self, jvm: &'gc JVMState<'gc>, int_state: &mut impl HasJavaStack<'gc>, wait_duration: Option<Duration>) -> Result<(), WasException<'gc>> {
        let mut guard = self.monitor2_priv.write().unwrap();
        let now = Instant::now();
    ...
    pub fn notify_reacquire<'gc, 'l>(&self, jvm: &'gc JVMState<'gc>, int_state: &mut impl HasJavaStack<'gc>, prev_count: usize) -> Result<(), WasException<'gc>> {
