...
use crate::class_loading::{
    assert_inited_or_initing_class, check_initing_or_inited_class, check_loaded_class,
};
use crate::interpreter::{run_function, safepoint_check, WasException};
use crate::interpreter_state::{
    CURRENT_INT_STATE_GUARD, CURRENT_INT_STATE_GUARD_VALID, InterpreterState,
};
...
pub struct ThreadState<'gc_life> {
    ...
    pub(crate) int_state_guard:
    &'static LocalKey<RefCell<Option<*mut InterpreterStateGuard<'static, 'static>>>>,
...
impl<'gc_life> ThreadState<'gc_life> {
    ...
    pub fn setup_main_thread(
        ...
        main_thread: &'gc_life Arc<JavaThread<'gc_life>>,
    ) -> Sender<MainThreadStartInfo> {
        ...
        main_thread.clone().underlying_thread.start_thread(
            box move |_| {
                ...
                main_thread
                    ...
                    .as_ref()
                    .unwrap()
                    .set_priority(JVMTI_THREAD_NORM_PRIORITY as i32);
                ...
                let mut int_state = InterpreterStateGuard::new(
                    jvm,
                    main_thread.clone(),
                    main_thread.interpreter_state.write().unwrap().into(),
                );
                main_thread.notify_alive(jvm); //is this too early?
                int_state.register_interpreter_state_guard(jvm);
                jvm.jvmti_state()
                    .map(|jvmti| jvmti.built_in_jdwp.agent_load(jvm, &mut int_state)); // technically this is to late and should have been called earlier, but needs to be on this thread.
                ...
                if let Some(jvmti) = jvm.jvmti_state() {
                    jvmti
                        .built_in_jdwp
                        .vm_inited(jvm, &mut int_state, main_thread.clone())
                }
                ...
                if let Some(jvmti) = jvm.jvmti_state() {
                    jvmti.built_in_jdwp.thread_start(
                        jvm,
                        &mut int_state,
                        main_thread.thread_object(),
                    )
                }
                let push_guard = int_state.push_frame(
                    StackEntry::new_completely_opaque_frame(LoaderName::BootstrapLoader, vec![]),
                    jvm,
                ); //todo think this is correct, check
                //handle any excpetions from here
                int_state.pop_frame(jvm, push_guard, false);
                let main_frame_guard = int_state.push_frame(
                    StackEntry::new_completely_opaque_frame(LoaderName::BootstrapLoader, vec![]),
                    jvm,
                );
    ...
    fn jvm_init_from_main_thread(
        ...
        let method_views =
            system_view.lookup_method_name(MethodName::method_initializeSystemClass());
        ...
        let initialize_system_frame = StackEntry::new_java_frame(
            jvm,
            system_class.clone(),
            init_method_view.method_i() as u16,
            locals,
        );
        ...
        let key = JString::from_rust(
            jvm,
            int_state,
            Wtf8Buf::from_string("java.home".to_string()),
        )
            .expect("todo");
        ...
        System::props(jvm, int_state)
            .set_property(jvm, int_state, key, value)
            .expect("todo");
    ...
    pub fn bootstrap_main_thread(
        jvm: &'vm_life JVMState<'vm_life>,
        threads: &'vm_life Threads<'vm_life>,
    ) -> Arc<JavaThread<'vm_life>> {
        let bootstrap_underlying_thread =
            threads.create_thread("Bootstrap Thread".to_string().into());
        let bootstrap_thread = Arc::new(JavaThread {
            java_tid: 0,
            underlying_thread: bootstrap_underlying_thread,
            thread_object: RwLock::new(None),
            interpreter_state: RwLock::new(InterpreterState::new(
                jvm,
                Box::into_raw(box JavaStatus::default()),
            )),
        ...
        jvm.thread_state
            .set_current_thread(bootstrap_thread.clone());
        bootstrap_thread.notify_alive(jvm);
        let mut new_int_state = InterpreterStateGuard::new(
            jvm,
            bootstrap_thread.clone(),
            bootstrap_thread.interpreter_state.write().unwrap().into(),
        );
        new_int_state.register_interpreter_state_guard(jvm);
        unsafe {
            jvm.native_libaries.load(
                jvm,
                &mut new_int_state,
                &jvm.native_libaries.libjava_path,
                "java".to_string(),
            );
            {
                ...
                let setup_hack_symbol: Symbol<
                    unsafe extern "system" fn(*const JNIInvokeInterface_),
                > = libjava_native_lib
                    .library
                    .get("setup_jvm_pointer_hack".as_bytes())
                    .unwrap();
        ...
        let thread_classfile =
            check_initing_or_inited_class(jvm, &mut new_int_state, CClassName::thread().into())
                .expect("couldn't load thread class");
        ...
        *bootstrap_thread.thread_object.write().unwrap() = thread_object.into();
        let thread_group_class = check_initing_or_inited_class(
            jvm,
            &mut new_int_state,
            CClassName::thread_group().into(),
        )
            .expect("couldn't load thread group class");
        let system_thread_group =
            JThreadGroup::init(jvm, &mut new_int_state, thread_group_class).expect("todo");
        *jvm.thread_state.system_thread_group.write().unwrap() = system_thread_group.clone().into();
        let main_jthread = JThread::new(
            jvm,
            &mut new_int_state,
            system_thread_group,
            "Main".to_string(),
        ...
        JavaThread::new(
            jvm,
            main_jthread,
            threads.create_thread("Main Java Thread".to_string().into()),
            false,
        )
    }
    ...
    pub fn get_current_thread_name(&self, jvm: &'gc_life JVMState<'gc_life>) -> String {
        ...
        thread_object
            .as_ref()
            .map(|jthread| jthread.name(jvm).to_rust_string(jvm))
            .unwrap_or(
                std::thread::current()
                    .name()
                    .unwrap_or("unknown")
    ...
    pub fn try_get_current_thread(&self) -> Option<Arc<JavaThread<'gc_life>>> {
        self.current_java_thread
            .with(|thread_refcell| unsafe { transmute(thread_refcell.borrow().clone()) })
    }
    ...
    pub fn start_thread_from_obj(
        ...
        let underlying = self
            .threads
            .create_thread(obj.name(jvm).to_rust_string(jvm).into());

        let (send, recv) = channel();
        let java_thread: Arc<JavaThread<'gc_life>> =
            JavaThread::new(jvm, obj.clone(), underlying, invisible_to_java);
        let loader_name = obj
    ...
    fn foo(
        jvm: &'gc_life JVMState<'gc_life>,
        java_thread: Arc<JavaThread<'gc_life>>,
        loader_name: LoaderName,
    ) {
        let java_thread_clone: Arc<JavaThread<'gc_life>> = java_thread.clone();
        let option: Option<RwLockWriteGuard<'_, InterpreterState>> =
            java_thread.interpreter_state.write().unwrap().into();
        let mut interpreter_state_guard: InterpreterStateGuard<'gc_life, '_> =
            InterpreterStateGuard::new(jvm, java_thread_clone, option); // { int_state: java_thread.interpreter_state.write().unwrap().into(), thread: &java_thread };
        interpreter_state_guard.register_interpreter_state_guard(jvm);

        if let Some(jvmti) = jvm.jvmti_state() {
            jvmti.built_in_jdwp.thread_start(
                jvm,
                &mut interpreter_state_guard,
                java_thread.clone().thread_object(),
            )
        }

        let frame_for_run_call = interpreter_state_guard.push_frame(
            StackEntry::new_completely_opaque_frame(loader_name, vec![]),
            jvm,
        );
        if let Err(WasException {}) = java_thread
            .thread_object
            .read()
            .unwrap()
            .as_ref()
            .unwrap()
            .run(jvm, &mut interpreter_state_guard)
        {
            JavaValue::Object(todo!() /*interpreter_state_guard.throw()*/)
                .cast_throwable()
                .print_stack_trace(jvm, &mut interpreter_state_guard)
                .expect(
                    "Exception occured while printing exception. Something is pretty messed up",
                );
        ...
        if let Err(WasException {}) = java_thread
            ...
            .as_ref()
            .unwrap()
            .exit(jvm, &mut interpreter_state_guard)
        {
    ...
    pub fn get_all_threads(
        &self,
    ) -> RwLockReadGuard<HashMap<JavaThreadId, Arc<JavaThread<'gc_life>>>> {
...
impl<'gc_life> JavaThread<'gc_life> {
    ...
    pub fn new(
        jvm: &'gc_life JVMState<'gc_life>,
        thread_obj: JThread<'gc_life>,
        underlying: Thread<'gc_life>,
        invisible_to_java: bool,
    ) -> Arc<JavaThread<'gc_life>> {
    ...
    fn update_thread_object(
        &self,
        jvm: &'gc_life JVMState<'gc_life>,
        status: RwLockWriteGuard<ThreadStatus>,
    ) {
        if self.thread_object.read().unwrap().is_some() {
            ...
            obj.set_thread_status(
                ...
                self.safepoint_state
                    .get_thread_status_number(status.deref()),
            )
        }
    }
    ...
    pub fn status_number(&self) -> jint {
        ...
        self.safepoint_state
            .get_thread_status_number(status_guard.deref())
    }
