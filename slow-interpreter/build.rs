...
fn main() {
    ...
    let std_arg = bindgen::Builder::default().header("stdarg-wrapper.h").parse_callbacks(Box::new(bindgen::CargoCallbacks)).derive_debug(true).rustfmt_bindings(true).generate().unwrap();
    ...
    std_arg.write_to_file(PathBuf::from("gen/stdarg.rs")).expect("Couldn't write bindings!");
}
