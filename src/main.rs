...
fn main() {
    ...
    let mut properties: Vec<String> = vec![
        "java.security.egd".to_string(),
        "file:/dev/urandom".to_string(),
    ];
    ...
    let mut libjdwp: OsString = OsString::from_str("/home/francis/build/openjdk-jdk8u/build/linux-x86_64-normal-server-release/jdk/lib/amd64/libjdwp.so").unwrap();
    {
        ...
        ap.refer(&mut debug)
            .add_option(&["-v", "--verbose"], StoreTrue, "Log debug info");
        ap.refer(&mut main_class_name)
            .add_option(&["--main"], Store, "Main class");
        ap.refer(&mut class_entries).add_option(
            &["--classpath"],
            List,
            "A list of directories from which to load classes",
        );
        ap.refer(&mut args)
        ...
        ap.refer(&mut enable_tracing)
            .add_option(&["--tracing"], StoreTrue, "Enable debug tracing");
        ap.refer(&mut enable_jvmti)
            .add_option(&["--jvmti"], StoreTrue, "Enable JVMTI");
        ap.refer(&mut properties)
            .add_option(&["--properties"], List, "Set JVM Properties");
        ap.refer(&mut unittest_mode).add_option(
            &["--unittest-mode"],
            StoreTrue,
            "Enable Unittest mode. This causes the main class to be ignored",
        );
        ap.refer(&mut store_generated_options).add_option(
            &["--store-anon-class"],
            StoreTrue,
            "Enables writing out of classes defined with Unsafe.defineClass",
        );
        ap.refer(&mut debug_print_exceptions).add_option(
            &["--debug-exceptions"],
            StoreTrue,
            "print excpetions even if caught",
        );
        ap.refer(&mut assertions_enabled)
            .add_option(&["--ea"], StoreTrue, "enable assertions");
        ap.parse_args_or_exit();
    }

    let classpath =
        Classpath::from_dirs(class_entries.iter().map(|x| Path::new(x).into()).collect());
    let main_class_name = ClassName::Str(main_class_name.replace('.', "/"));
    let jvm_options = JVMOptions::new(
        main_class_name,
        classpath,
        args,
        libjava,
...
fn within_thread_scope<'l>(scope: Scope<'l>, jvm_options: JVMOptions, gc: &'l GC<'l>) {
    ...
    let main_thread: Arc<JavaThread> =
        ThreadState::bootstrap_main_thread(jvm_ref, &jvm_ref.thread_state.threads);
