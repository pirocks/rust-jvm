...
pub fn get_stack_map_frames(vf: &VerifierContext, class: &ClassWithLoader, method_info: &MethodView) -> Vec<StackMap> {
    ...
    for (_, entry) in stack_map.iter().enumerate() {
        ...
        res.push(StackMap {
            ...
            map_frame: Frame {
                locals: Rc::new(expand_to_length(frame.locals.clone(), frame.max_locals as usize, VType::TopType)
                    .iter()
                    .cloned()
                    .collect()),
...
//so these parameter types come unconverted and therefore need conversion
    for parameter_type in parameter_types {
        add_verification_type_to_array_convert(&mut locals, &parameter_type.to_verification_type(LoaderName::BootstrapLoader))//todo fix bootstrap loader
    }
