...
use crate::verifier::filecorrectness::{
    does_not_override_final_method, is_assignable, super_class_chain,
};
...
pub fn valid_type_transition(
    env: &Environment,
    expected_types_on_stack: Vec<VType>,
    result_type: VType,
    input_frame: Frame,
) -> Result<Frame, TypeSafetyError> {
    let Frame {
        locals,
        stack_map: input_operand_stack,
        flag_this_uninit,
    } = input_frame;
    let interim_operand_stack =
        pop_matching_list(&env.vf, input_operand_stack, expected_types_on_stack)?;
    ...
    if operand_stack_has_legal_length(env, &next_operand_stack) {
        ...
        Result::Err(TypeSafetyError::NotSafe(
            "Operand stack did not have legal length".to_string(),
        ))
    }
}

pub fn pop_matching_list(
    vf: &VerifierContext,
    pop_from: OperandStack,
    pop: Vec<VType>,
) -> Result<OperandStack, TypeSafetyError> {
    pop_matching_list_impl(vf, pop_from, pop.as_slice())
}

pub fn pop_matching_list_impl(
    vf: &VerifierContext,
    mut pop_from: OperandStack,
    pop: &[VType],
) -> Result<OperandStack, TypeSafetyError> {
    if pop.is_empty() {
        Result::Ok(pop_from)
...
pub fn pop_matching_type<'l>(
    vf: &VerifierContext,
    operand_stack: &'l mut OperandStack,
    type_: &VType,
...
pub fn push_operand_stack(
    vf: &VerifierContext,
    mut operand_stack: OperandStack,
    type_: &VType,
) -> OperandStack {
    match type_ {
...
pub fn operand_stack_has_legal_length(
...
pub fn frame_is_assignable(
    vf: &VerifierContext,
    left: &Frame,
    right: &Frame,
) -> Result<(), TypeSafetyError> {
    let locals_assignable_res: Result<Vec<_>, _> = left
        .locals
        .iter()
        .zip(right.locals.iter())
        .map(|(left_, right_)| is_assignable(vf, left_, right_))
        .collect();
    let locals_assignable = locals_assignable_res.is_ok();
    let stack_assignable_res: Result<Vec<_>, _> = left
        .stack_map
        .iter()
        .zip(right.stack_map.iter())
        .map(|(left_, right_)| is_assignable(vf, left_, right_))
        .collect();
...
pub fn method_with_code_is_type_safe<'l, 'k>(
    vf: &'l mut VerifierContext<'k>,
    class: ClassWithLoader,
    method: ClassWithLoaderMethod,
) -> Result<(), TypeSafetyError> {
    ...
    let mut instructs: Vec<&CInstruction> = code
        ...
        .collect();
    let end_of_code = CInstruction {
        offset: final_offset,
        instruction_size: 0,
        info: CompressedInstructionInfo::EndOfCode,
    };
...
pub fn handler_exception_class(
    _vf: &VerifierContext,
    handler: &Handler,
    loader: LoaderName,
) -> ClassWithLoader {
...
fn merge_stack_map_and_code_impl<'l>(
    instructions: &[&'l CInstruction],
    stack_maps: &[&'l StackMap],
    res: &mut Vec<MergedCodeInstruction<'l>>,
) {
    if stack_maps.is_empty() {
        res.append(
            &mut instructions
                .iter()
                .map(|x| MergedCodeInstruction::Instruction(x))
                .collect(),
        );
        return;
    }
 ...
 */
pub fn merge_stack_map_and_code<'l>(
    instruction: Vec<&'l CInstruction>,
    stack_maps: Vec<&'l StackMap>,
) -> Vec<MergedCodeInstruction<'l>> {
    let mut res = vec![];
    merge_stack_map_and_code_impl(instruction.as_slice(), stack_maps.as_slice(), &mut res);
    res
}

fn method_initial_stack_frame(
    vf: &VerifierContext,
    class: &ClassWithLoader,
    method: &ClassWithLoaderMethod,
    frame_size: u16,
) -> Result<(Frame, VType), TypeSafetyError> {
...
fn method_initial_this_type(
    vf: &VerifierContext,
    class: &ClassWithLoader,
    method: &ClassWithLoaderMethod,
) -> Result<Option<VType>, TypeSafetyError> {
    let method_class = get_class(vf, &method.class)?;
    let method_view = method_class.method_view_i(method.method_index as u16);
    if method_view.is_static() {
        let method_name = method_view.name();
        if method_name != MethodName::constructor_init() {
            //todo convert init to classname like string/id
            Ok(None)
        } else {
...
fn instance_method_initial_this_type(
    vf: &VerifierContext,
    class: &ClassWithLoader,
    method: &ClassWithLoaderMethod,
) -> Result<VType, TypeSafetyError> {
    ...
    if method_name == MethodName::constructor_init() {
        if class.class_name == CClassName::object() {
            Result::Ok(VType::Class(ClassWithLoader {
                class_name: class.class_name,
                loader: class.loader.clone(),
            }))
        } else {
            let mut chain = vec![];
        ...
        Result::Ok(
            get_class(vf, class)?
                .name()
                .to_verification_type(class.loader),
        )
    }
}