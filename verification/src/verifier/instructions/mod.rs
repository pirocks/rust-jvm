...
use rust_jvm_common::compressed_classfile::code::{
    CInstructionInfo, CompressedLdc2W, CompressedLdcW,
};
...
use crate::verifier::codecorrectness::{
    Environment, frame_is_assignable, Handler,
    handler_exception_class, init_handler_is_legal, MergedCodeInstruction, operand_stack_has_legal_length,
    push_operand_stack, size_of, valid_type_transition,
};
...
pub fn merged_code_is_type_safe(
    env: &mut Environment,
    merged_code: &[MergedCodeInstruction],
    after_frame: FrameResult,
        ...
        MergedCodeInstruction::Instruction(i) => {
            ...
            match instruction_is_type_safe(&i, env, i.offset, f)? {
                InstructionTypeSafe::Safe(s) => {
                    ...
                    let _exception_stack_frame1 =
                        instruction_satisfies_handlers(env, i.offset, &exception_frame)?;
                    merged_code_is_type_safe(env, rest, FrameResult::Regular(next_frame))
                }
                InstructionTypeSafe::AfterGoto(ag) => {
                    let _exception_stack_frame1 =
                        instruction_satisfies_handlers(env, i.offset, &ag.exception_frame)?;
...
fn instruction_satisfies_handlers(
    env: &Environment,
    offset: u16,
    exception_stack_frame: &Frame,
) -> Result<(), TypeSafetyError> {
    let handlers = &env.handlers;
    let applicable_handler = handlers.iter().filter(|h| is_applicable_handler(offset, h));
    let res: Result<Vec<_>, _> = applicable_handler
        .map(|h| instruction_satisfies_handler(env, exception_stack_frame, h))
        .collect();
    res?;
    Result::Ok(())
}
...
fn instruction_satisfies_handler(
    env: &Environment,
    exc_stack_frame: &Frame,
    handler: &Handler,
) -> Result<(), TypeSafetyError> {
    ...
    let stack_map =
        OperandStack::new_prolog_display_order(&[class_to_type(&env.vf, &exception_class)?]);
    ...
    if operand_stack_has_legal_length(env, &stack_map) {
        ...
        Result::Err(TypeSafetyError::NotSafe(
            "operand stack does not have legal length".to_string(),
        ))
    }
}
...
pub fn handlers_are_legal(env: &Environment) -> Result<(), TypeSafetyError> {
    let handlers = &env.handlers;
    let res: Result<Vec<_>, _> = handlers.iter().map(|h| handler_is_legal(env, h)).collect();
    res?;
    Result::Ok(())
}
    ...
    if h.start < h.end {
        if start_is_member_of(h.start, env.merged_code.unwrap()) {
            ...
            if instructions_include_end(env.merged_code.unwrap(), h.end) {
                let exception_class =
                    handler_exception_class(&env.vf, &h, env.class_loader.clone());
                is_assignable(
                    &env.vf,
                    &VType::Class(ClassWithLoader {
                        class_name: exception_class.class_name,
                        loader: env.class_loader.clone(),
                    }),
                    &VType::Class(ClassWithLoader {
                        class_name: CClassName::throwable(),
                        loader: LoaderName::BootstrapLoader,
                    }),
                )?;
                init_handler_is_legal(env, h)
            } else {
                Result::Err(TypeSafetyError::NotSafe(
                    "Instructions do not include handler end".to_string(),
                ))
            }
        } else {
            Result::Err(TypeSafetyError::NotSafe(
                "No instruction found at handler start.".to_string(),
            ))
        }
    } else {
        Result::Err(TypeSafetyError::NotSafe(
            "Handler start not less than end".to_string(),
        ))
    }
}
...
pub fn instruction_is_type_safe_dup(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    let Frame {
        locals,
        stack_map: input_operand_stack,
        flag_this_uninit: flags,
    } = stack_frame;
...
pub fn can_safely_push(
    env: &Environment,
    input_operand_stack: OperandStack,
    type_: &VType,
) -> Result<OperandStack, TypeSafetyError> {
...
pub fn pop_category1(
    vf: &VerifierContext,
    input: &mut OperandStack,
) -> Result<VType, TypeSafetyError> {
...
pub fn instruction_is_type_safe_dup_x1(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    let Frame {
...
pub fn can_safely_push_list(
    env: &Environment,
    input_stack: OperandStack,
    types: Vec<VType>,
) -> Result<OperandStack, TypeSafetyError> {
...
pub fn instruction_is_type_safe_dup2(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    let Frame {
        locals,
        stack_map: input_stack,
        flag_this_uninit: flags,
    } = stack_frame;
...
pub fn instruction_is_type_safe_dup_x2(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    let Frame {
...
fn dup_x2_form_is_type_safe(
    env: &Environment,
    mut input_stack: OperandStack,
) -> Result<OperandStack, TypeSafetyError> {
...
pub fn instruction_is_type_safe_dup2_x1(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    let Frame {
...
pub fn dup2_x1form_is_type_safe(
    env: &Environment,
    input_frame: OperandStack,
) -> Result<OperandStack, TypeSafetyError> {
    dup2_x1form1_is_type_safe(env, input_frame.clone())
        .or_else(|_| dup2_x1form2_is_type_safe(env, input_frame))
}
...
pub fn dup2_x1form1_is_type_safe(
    env: &Environment,
    input_frame: OperandStack,
) -> Result<OperandStack, TypeSafetyError> {
    ...
    can_safely_push_list(
        env,
        rest,
        vec![type2.clone(), type1.clone(), type3, type2, type1],
    )
}
...
pub fn dup2_x1form2_is_type_safe(
    env: &Environment,
    input_frame: OperandStack,
) -> Result<OperandStack, TypeSafetyError> {
    let mut stack1 = input_frame;
...
pub fn instruction_is_type_safe_dup2_x2(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    let Frame {
...
pub fn dup2_x2form_is_type_safe(
    env: &Environment,
    input_frame: OperandStack,
) -> Result<OperandStack, TypeSafetyError> {
    dup2_x2form1_is_type_safe(env, input_frame.clone()).or_else(|_| {
        dup2_x2form2_is_type_safe(env, input_frame.clone()).or_else(|_| {
            dup2_x2form3_is_type_safe(env, input_frame.clone())
                .or_else(|_| dup2_x2form4_is_type_safe(env, input_frame))
        })
    })
}
...
pub fn dup2_x2form1_is_type_safe(
    env: &Environment,
    mut input_stack: OperandStack,
) -> Result<OperandStack, TypeSafetyError> {
...
pub fn dup2_x2form2_is_type_safe(
    env: &Environment,
    mut input_stack: OperandStack,
) -> Result<OperandStack, TypeSafetyError> {
    ...
    can_safely_push_list(
        env,
        input_stack,
        vec![type_1.clone(), type_3, type_2, type_1],
    )
}
...
pub fn dup2_x2form3_is_type_safe(
    env: &Environment,
    mut input_stack: OperandStack,
) -> Result<OperandStack, TypeSafetyError> {
    let type_1 = pop_category1(&env.vf, &mut input_stack)?;
    ...
    can_safely_push_list(
        env,
        input_stack,
        vec![type_2.clone(), type_1.clone(), type_3, type_2, type_1],
    )
}
...
pub fn dup2_x2form4_is_type_safe(
    env: &Environment,
    mut input_stack: OperandStack,
) -> Result<OperandStack, TypeSafetyError> {
    let type_1 = pop_category2(&env.vf, &mut input_stack)?;
...
pub fn instruction_is_type_safe_i2d(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_i2f(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_i2l(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_iadd(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    type_transition(
        env,
        stack_frame,
        vec![VType::IntType, VType::IntType],
        VType::IntType,
    )
}

pub fn instruction_is_type_safe_iinc(
    index: u16,
    _env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    ...
    match should_be_int {
        VType::IntType => Result::Ok(InstructionTypeSafe::Safe(ResultFrames {
            next_frame: Frame {
...
pub fn instruction_is_type_safe_ineg(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_l2d(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_l2f(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_l2i(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_ladd(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    type_transition(
        env,
        stack_frame,
        vec![VType::LongType, VType::LongType],
        VType::LongType,
    )
}

fn instruction_is_type_safe_lcmp(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    type_transition(
...
pub fn loadable_constant(vf: &VerifierContext, c: &CompressedLdcW) -> VType {
    match c {
        ...
        CompressedLdcW::MethodHandle {} => VType::Class(ClassWithLoader {
            class_name: CClassName::method_handle(),
            loader: vf.current_loader.clone(),
        }),
        CompressedLdcW::MethodType {} => VType::Class(ClassWithLoader {
            class_name: CClassName::method_type(),
            loader: vf.current_loader.clone(),
        }),
        CompressedLdcW::LiveObject(idx) => vf
            .live_pool_getter
            .elem_type(*idx)
            .to_verification_type(vf.current_loader),
    }
}
...
pub fn instruction_is_type_safe_ldc_w(
    const_: &CompressedLdcW,
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    ...
    let type_ = match const_ {
        CompressedLdcW::Integer { .. } => VType::IntType,
        CompressedLdcW::Float { .. } => VType::FloatType,
        CompressedLdcW::Class { .. } => VType::Class(ClassWithLoader {
            class_name: CClassName::class(),
            loader: env.vf.current_loader.clone(),
        }),
        CompressedLdcW::String { .. } => VType::Class(ClassWithLoader {
            class_name: CClassName::string(),
            loader: env.vf.current_loader.clone(),
        }),
        CompressedLdcW::MethodType {} => VType::Class(ClassWithLoader {
            class_name: CClassName::method_type(),
...
pub fn instruction_is_type_safe_ldc2_w(
    const_: &CompressedLdc2W,
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    let type_: VType = loadable_constant_w(&env.vf, const_);
...
pub fn instruction_is_type_safe_lneg(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_lshl(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    type_transition(
        env,
        stack_frame,
        vec![VType::IntType, VType::LongType],
        VType::LongType,
    )
}

pub fn instruction_is_type_safe_pop(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    let Frame {
...
pub fn instruction_is_type_safe_pop2(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    let Frame {
        locals,
        stack_map: operand_stack,
        flag_this_uninit: flags,
    } = stack_frame;
...
fn pop2form_is_type_safe(
    env: &Environment,
    mut input: OperandStack,
) -> Result<OperandStack, TypeSafetyError> {
    let first_pop = input.operand_pop();
    let second_pop = input.operand_pop();
    let succeds = match first_pop {
        VType::TopType => size_of(&env.vf, &second_pop) == 2,
        _ => size_of(&env.vf, &first_pop) == 1 && size_of(&env.vf, &second_pop) == 1,
    };
    if succeds {
        Result::Ok(input)
    } else {
...
pub fn instruction_is_type_safe_sipush(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_nop(
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    let Frame {
        locals,
...
pub fn instruction_is_type_safe_swap(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    let locals = &stack_frame.locals;
...
pub fn exception_stack_frame(stack_frame_locals: Rc<Vec<VType>>, stack_frame_flag: bool) -> Frame {
    Frame {
        locals: stack_frame_locals,
        stack_map: OperandStack::empty(),
        flag_this_uninit: stack_frame_flag,
    }
}