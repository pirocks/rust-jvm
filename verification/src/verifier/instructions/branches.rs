...
use rust_jvm_common::compressed_classfile::{
    CMethodDescriptor, CompressedClassfileStringPool, CPDType, CPRefType,
};
...
use crate::verifier::codecorrectness::{
    can_pop, Environment, MergedCodeInstruction, push_operand_stack,
};
...
use crate::verifier::instructions::{
    AfterGotoFrames, exception_stack_frame, InstructionTypeSafe, ResultFrames, target_is_type_safe,
};
use crate::verifier::passes_protected_check;
use crate::verifier::TypeSafetyError;

pub fn instruction_is_type_safe_return(
    env: &Environment,
    stack_frame: Frame,
    ...
    let exception_frame =
        exception_stack_frame(stack_frame.locals.clone(), stack_frame.flag_this_uninit);
    Result::Ok(InstructionTypeSafe::AfterGoto(AfterGotoFrames {
        exception_frame,
    }))
}

pub fn type_safe_if_cmp(
    target: u16,
    env: &Environment,
    stack_frame: Frame,
    comparison_types: Vec<VType>,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_if_acmpeq(
    target: u16,
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    type_safe_if_cmp(
        ...
        env,
        stack_frame,
        vec![VType::Reference, VType::Reference],
    )
}

pub fn instruction_is_type_safe_goto(
    target: u16,
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_ireturn(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    ...
    match env.return_type {
        ...
        _ => {
            return Result::Err(TypeSafetyError::NotSafe(
                "Tried to return not an int with ireturn".to_string(),
            ))
        }
    }
    let locals = stack_frame.locals.clone();
...
pub fn instruction_is_type_safe_areturn(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_if_icmpeq(
    target: u16,
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_ifeq(
    target: u16,
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_ifnonnull(
    target: u16,
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_invokedynamic(
    cp: usize,
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    ...
    let (call_site_name, descriptor) = match &method_class.constant_pool_view(cp) {
        ConstantInfoView::InvokeDynamic(i) => (
            MethodName(i.name_and_type().name(env.vf.string_pool)),
            i.name_and_type().desc_method(env.vf.string_pool),
        ),
        _ => panic!(),
    };
    if call_site_name == MethodName::constructor_init()
        || call_site_name == MethodName::constructor_clinit()
    {
        return Result::Err(TypeSafetyError::NotSafe(
            "Tried to invoke dynamic in constructor".to_string(),
        ));
    }
    let operand_arg_list: Vec<VType> = descriptor
        .arg_types
        .iter()
        .rev()
    ...
    let return_type = descriptor
        .return_type
        .to_verification_type(env.class_loader);
...
pub fn instruction_is_type_safe_invokeinterface(
    ...
    {
        return Result::Err(TypeSafetyError::NotSafe(
            "Tried to invoke interface on constructor".to_string(),
        ));
    }
    let mut operand_arg_list: Vec<_> = descriptor
        .arg_types
        .iter()
        .rev()
        .map(|x| x.to_verification_type(env.class_loader))
...
fn count_is_valid(
    count: usize,
    input_frame_stack_map_size: usize,
    output_frame: &Frame,
...
pub fn instruction_is_type_safe_invokespecial(
    method_class_type: &CPDType,
    method_name: MethodName,
...
fn invoke_special_init(
    env: &Environment,
    stack_frame: Frame,
    method_class_name: CClassName,
    parsed_descriptor: &CMethodDescriptor,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    let mut stack_arg_list: Vec<_> = parsed_descriptor
        .arg_types
        .iter()
        .map(|x| x.to_verification_type(env.class_loader))
        .collect();
    ...
    match first {
        VType::Uninitialized(address) => {
            let uninit_address = VType::Uninitialized(UninitializedVariableInfo {
                offset: address.offset,
            });
            let this = rewritten_uninitialized_type(
                &uninit_address,
                env,
                &ClassWithLoader {
                    class_name: method_class_name,
                    loader: current_class_loader,
                },
            )?;
            let next_flags = rewritten_initialization_flags(&uninit_address, flags);
            let this_class = VType::Class(this);
            let next_operand_stack =
                substitute_operand_stack(&uninit_address, &this_class, &operand_stack);
            ...
            passes_protected_check(
                env,
                method_class_name,
                MethodName::constructor_init().0,
        ...
        VType::UninitializedThis => {
            ...
            let this_class = VType::Class(this);
            let next_operand_stack =
                substitute_operand_stack(&VType::UninitializedThis, &this_class, &operand_stack);
...
pub fn substitute(old: &VType, new: &VType, list: &[VType]) -> Vec<VType> {
    list.iter()
        .map(|x| (if old == x { new } else { x }).clone())
        .collect()
}
...
fn rewritten_uninitialized_type(
    type_: &VType,
    env: &Environment,
    _class: &ClassWithLoader,
) -> Result<ClassWithLoader, TypeSafetyError> {
    match type_ {
        VType::Uninitialized(address) => match env.merged_code {
            None => unimplemented!(),
            Some(code) => {
                ...
                match found_new {
                    ...
                    Some(new_this) => match new_this {
                        MergedCodeInstruction::Instruction(instr) => match &instr.info {
                            CInstructionInfo::new(class_name) => Result::Ok(ClassWithLoader {
        ...
        VType::UninitializedThis => {
            ...
            Result::Ok(ClassWithLoader {
                class_name: env.method.class.class_name.clone(),
                loader: env.method.class.loader.clone(),
            })
        }
        _ => {
            panic!()
...
fn invoke_special_not_init(
    ...
    if method_name == MethodName::constructor_clinit() {
        return Result::Err(TypeSafetyError::NotSafe(
            "invoke special on clinit is not allowed".to_string(),
        ));
    }
    ...
    let mut operand_arg_list_copy: Vec<_> = parsed_descriptor
        .arg_types
        .iter()
        .rev()
        .map(|x| x.to_verification_type(env.class_loader))
    ...
    let return_type = parsed_descriptor
        .return_type
        .to_verification_type(env.class_loader);
    let locals = stack_frame.locals.clone();
    let flag = stack_frame.flag_this_uninit;
    let mut operand_arg_list_copy2: Vec<_> = parsed_descriptor
        .arg_types
        .iter()
        .rev()
        .map(|x| x.to_verification_type(env.class_loader))
...
pub fn instruction_is_type_safe_invokestatic(
    method_name: MethodName,
    parsed_descriptor: &CMethodDescriptor,
    env: &Environment,
    stack_frame: Frame,
    ...
    if method_name_str.contains("arrayOf")
        || method_name_str.contains('[')
        || method_name == MethodName::constructor_init()
        || method_name == MethodName::constructor_clinit()
    {
        unimplemented!();
    }
    let operand_arg_list: Vec<_> = parsed_descriptor
        .arg_types
        .iter()
        .map(|x| x.to_verification_type(env.class_loader))
        .collect();
    ...
    //todo redundant?
    let stack_arg_list: Vec<_> = operand_arg_list.iter().rev().cloned().collect();
    let return_type = parsed_descriptor
        .return_type
        .to_verification_type(env.class_loader);
    if method_name == MethodName::method_linkToStatic()
        || method_name == MethodName::method_linkToVirtual()
    {
...
pub fn instruction_is_type_safe_invokevirtual(
    class_type: &CPDType,
    method_name: MethodName,
    parsed_descriptor: &CMethodDescriptor,
    env: &Environment,
    ...
    let return_type = parsed_descriptor
        .return_type
        .to_verification_type(env.class_loader); //todo what should the loader here be?
    let locals = stack_frame.locals.clone();
    let flag = stack_frame.flag_this_uninit;
...
pub fn get_method_descriptor(
    ...
    cp: usize,
    classfile: &dyn ClassView,
) -> (CPDType, MethodName, CMethodDescriptor) {
    ...
    let (ref_type, method_name, parsed_descriptor) = match c {
        ConstantInfoView::Methodref(m) => {
            ...
            let (method_name, descriptor) = (
                m.name_and_type().name(pool),
                m.name_and_type().desc_method(pool),
            );
...
pub fn possibly_array_to_type(class_name: &str) -> ReferenceTypeView {
    if class_name.contains('[') {
        ...
        ReferenceTypeView::Array(Box::new(PTypeView::from_ptype(
            &class_type.unwrap_array_type(),
        )))
    } else {
...
pub fn instruction_is_type_safe_lreturn(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
    match env.return_type {
...
pub fn instruction_is_type_safe_dreturn(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
...
pub fn instruction_is_type_safe_freturn(
    env: &Environment,
    stack_frame: Frame,
) -> Result<InstructionTypeSafe, TypeSafetyError> {
