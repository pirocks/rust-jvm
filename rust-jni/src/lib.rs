    ...
    fn call(&self, classfile: Arc<RuntimeClass>, method_i: usize, args: Vec<JavaValue>, return_type: ParsedType) -> Option<JavaValue> {
        ...
        match return_type {
            ParsedType::VoidType => {
                ...
                let jclass: jclass = unsafe { transmute(010101 as u64)/*MaybeUninit::zeroed().assume_init()*/ };
                unsafe {
                    let env: sys::JNIEnv = &sys::JNINativeInterface_ {
                        reserved0: unsafe { transmute(0xdeadbeaf as u64) },
                        ...
                        GetObjectRefType: unsafe { transmute(0xdeadbeaf as u64) },
                    };
                    dbg!(env);
                    dbg!(register_natives as *const ());
