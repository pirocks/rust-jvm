...
use rust_jvm_common::compressed_classfile::CFieldDescriptor;
...
pub fn putstatic<'vm>(
    ...
    match resolver.lookup_type_inited_initing(&target_class.into()) {
        ...
        Some((rc, _loader)) => {
            ...
            match put_static_from_class_class(method_frame_data, current_instr_data, field_name, restart_point.clone(), class_class, to_put) {
                None => {
                    panic!()
                }
                Some(res) => {
                    return Either::Right(array_into_iter([
                        restart_point,
...
fn put_static_from_class_class<'vm>(
    method_frame_data: &JavaCompilerMethodAndFrameData,
    current_instr_data: &CurrentInstructionCompilerData,
    field_name: FieldName,
    restart_point: IRInstr,
    ...
    let static_field_number_and_field_type = match class_class.static_field_numbers.get(&field_name) {
        ...
        None => {
            if let Some(parent) = class_class.parent.as_ref() {
                if let Some(res) = put_static_from_class_class(method_frame_data, current_instr_data, field_name, restart_point.clone(), parent.unwrap_class_class(), to_put) {
                    return Some(res);
                }
            }
            for interface_class in class_class.interfaces.iter() {
                if let Some(res) = put_static_from_class_class(method_frame_data, current_instr_data, field_name, restart_point.clone(), interface_class.unwrap_class_class(), to_put) {
                    return Some(res);
                }
            }
            return None;
