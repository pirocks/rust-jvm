...
fn array_load_impl(method_frame_data: &JavaCompilerMethodAndFrameData, current_instr_data: CurrentInstructionCompilerData, arr_type: CPDType) -> impl Iterator<Item=IRInstr> {
    ...
    array_into_iter([
        ...
        IRInstr::Const64bit {
            to: elem_0_offset_register,
            const_: array_layout.elem_0_entry_offset() as u64,
        },
        IRInstr::Load {
            to: length,
            from_address: array_ref,
            size: Size::int(),
        },
        IRInstr::Add {
            res: array_ref,
            a: elem_0_offset_register,
            size: Size::pointer(),
        },
        IRInstr::BoundsCheck {
            length,
            index,
            size: Size::int(),
            exit: IRVMExitType::ArrayOutOfBounds { java_pc: current_instr_data.current_offset, index: index_offset },
        },
        IRInstr::MulConst {
            res: index,
        ...
        IRInstr::StoreFPRelative {
            ...
            size: runtime_type_to_size(&arr_type.to_runtime_type().unwrap()),
        }
    ])
}
