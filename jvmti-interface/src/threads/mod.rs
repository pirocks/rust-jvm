...
macro_rules! get_thread_or_error {
    ($jvm: expr,$raw_thread: expr) => {
        match slow_interpreter::java_values::JavaValue::Object(from_object($jvm, $raw_thread)).try_cast_thread() {
...
pub unsafe extern "C" fn get_thread_info(env: *mut jvmtiEnv, thread: jthread, info_ptr: *mut jvmtiThreadInfo) -> jvmtiError {
    ...
    (*info_ptr).thread_group = new_local_ref_public(jvm.thread_state.get_system_thread_group().object().to_gc_managed().into(), todo!()/*int_state*/);
    ...
    let context_class_loader = new_local_ref_public(class_loader.map(|x| x.object().to_gc_managed()), todo!()/*int_state*/);
...
///     Native VM jni_interface functions, such as JNI and JVM TI functions, may be implemented as VM code.
...
///     	jvmtiError err;
///
///     	err = (*jvmti_interface)->GetThreadState(jvmti_interface, thread, &state);
