...
fn run_integration_test() {
    ...
    let rt_jar_ext_location = var("RTJAR_EXT_DIR").unwrap_or(DEFAULT_RT_JAR_EXT_DIR_LOCATION.to_string());
    let mut resources_path = current_dir().unwrap();
    resources_path.push("resources/test");

    let mut java_process = Command::new("target/debug/java").arg("--unittest-mode").arg("--libjava").arg(libjava_path).arg("--classpath").arg(rt_jar_location).arg(rt_jar_ext_location).arg("--jvmti").arg("--tracing").spawn().unwrap();
