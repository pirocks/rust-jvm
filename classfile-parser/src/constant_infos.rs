...
use rust_jvm_common::classfile::{
    Class, ConstantInfo, ConstantKind, Fieldref, Float, Integer, InterfaceMethodref,
    InvalidConstant, InvokeDynamic, Long, MethodHandle, Methodref, MethodType, NameAndType,
    String_, Utf8,
};
use rust_jvm_common::classfile::Double;
use rust_jvm_common::classfile::ReferenceKind::{
    GetField, GetStatic, InvokeInterface, InvokeSpecial, InvokeStatic, InvokeVirtual,
    NewInvokeSpecial, PutField, PutStatic,
};
...
pub fn parse_constant_info(
    ...
    _debug: bool,
) -> Result<ConstantInfo, ClassfileParsingError> {
    let kind = p.read8()?;
    let result_kind: ConstantKind = match kind {
        ...
        METHOD_TYPE_CONST_NUM => {
            let descriptor_index = p.read16()?;
            ConstantKind::MethodType(MethodType { descriptor_index })
        }
...
pub fn parse_constant_infos(
    p: &mut dyn ParsingContext,
    constant_pool_count: u16,
) -> Result<Vec<ConstantInfo>, ClassfileParsingError> {
    ...
    for i in 0..constant_pool_count {
        if skip_next_iter {
            constants.push(ConstantInfo {
                kind: (ConstantKind::InvalidConstant(InvalidConstant {})),
            });
