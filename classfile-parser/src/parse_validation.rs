...
use rust_jvm_common::classfile::{
    ACC_ABSTRACT, ACC_ANNOTATION, ACC_ENUM, ACC_FINAL, ACC_INTERFACE, ACC_PRIVATE,
    ACC_PROTECTED, ACC_PUBLIC, ACC_SUPER, ACC_VOLATILE, Annotation, AnnotationDefault, AnnotationValue,
    ArrayValue, AttributeInfo, AttributeType, BootstrapMethod, BootstrapMethods, Class,
    Classfile, ClassInfoIndex, Code, ConstantInfo, ConstantKind,
    ElementValue, ElementValuePair, EnclosingMethod, EnumConstValue,
    Exceptions, FieldInfo, Fieldref, InterfaceMethodref, InvokeDynamic, LocalVariableTableEntry, LocalVariableTypeTableEntry,
    LocalVarTargetTableEntry, MethodInfo, MethodParameter, MethodParameters, Methodref, NameAndType, ReferenceKind,
...
use crate::parse_validation::ClassfileError::{
    BadConstantPoolEntry, ExpectedClassEntry, ExpectedDoubleCPEntry, ExpectedFloatCPEntry,
    ExpectedIntegerCPEntry, ExpectedLongCPEntry, ExpectedNameAndType, ExpectedUtf8CPEntry,
...
impl ValidatorSettings {
    pub fn validate_parsed(&self, c: &Classfile) -> Result<(), ClassfileError> {
        ...
        if self.allowable_major_versions.contains(&c.major_version) {
            ...
            let allowable_minors = self
                .allowable_minor_versions
                .get(&c.major_version)
                .expect("allowable major version doesn't have matching allowable minor versions");
        ...
        for attr in &c.attributes {
            self.validate_attribute(
                &mut attribute_validation_context,
                attr,
                c,
                &AttributeEnclosingType::Class(c),
            )?;
        }
        Result::Ok(())
    }
    ...
    pub fn is_utf8_check<'l>(
        &self,
        cpi: u16,
        c: &'l Classfile,
    ) -> Result<&'l Utf8, ClassfileError> {
    ...
    pub fn is_class_check<'l>(
        &self,
        cpi: u16,
        c: &'l Classfile,
    ) -> Result<&'l Class, ClassfileError> {
        self.index_check(cpi, c)?;
        match &c.constant_pool[cpi as usize].kind {
    ...
    pub fn is_name_and_type_check<'l>(
        &self,
        cpi: u16,
        c: &'l Classfile,
    ) -> Result<&'l NameAndType, ClassfileError> {
        self.index_check(cpi, c)?;
        match &c.constant_pool[cpi as usize].kind {
    ...
    pub fn validate_constant_info(
        ...
        match &ci.kind {
            ...
            ConstantKind::InvokeDynamic(InvokeDynamic {
                ...
                if *bootstrap_method_attr_index as usize
                    >= bootstrap_attribute.bootstrap_methods.len()
                {
                    return Err(BadConstantPoolEntry);
                }
            }
            ...
            ConstantKind::LiveObject(_) => {
                return Result::Err(
                    ClassfileError::SomehowManagedToParseAnInMemoryOnlyFeatureOfClassfiles,
                );
            }
        }
        *ignore_next = false;
    ...
    pub fn validate_interface_method_ref(
        &self,
        c: &Classfile,
        fr: &InterfaceMethodref,
    ) -> Result<(), ClassfileError> {
    ...
    pub fn validate_attribute(
        &self,
        attribute_validation_context: &mut AttributeValidationContext,
        a: &AttributeInfo,
        c: &Classfile,
        attr: &AttributeEnclosingType,
    ) -> Result<(), ClassfileError> {
        match &a.attribute_type {
            ...
            AttributeType::LocalVariableTable(lvt) => match attr {
                AttributeEnclosingType::Code(code) => {
                    for LocalVariableTableEntry {
                        start_pc,
                        length,
                        name_index,
                        descriptor_index,
                        ..
                    } in &lvt.local_variable_table
                    {
            ...
            AttributeType::LocalVariableTypeTable(lvtt) => match attr {
                AttributeEnclosingType::Code(code) => {
                    for LocalVariableTypeTableEntry {
                        start_pc,
                        length,
                        name_index,
                        descriptor_index,
            ...
            AttributeType::Deprecated(_) => match attr {
                AttributeEnclosingType::Code(_) => {
            ...
            AttributeType::RuntimeInvisibleParameterAnnotations(annotations) => {
                ...
                attribute_validation_context.has_been_runtime_invisible_parameter_annotations =
                    true;
            }
            ...
            TargetInfo::ThrowsTarget { throws_type_index } => match attr {
                AttributeEnclosingType::Method(method) => {
                    let exceptions = method
                        .exception_attribute()
                        .ok_or(ClassfileError::MissingExceptionsAttribute)?;
    ...
    fn validate_annotation(
        ...
        for ElementValuePair {
            element_name_index,
            value,
        } in &annotation.element_value_pairs
        {
            self.validate_utf8(c, *element_name_index)?;
            self.validate_element_value(c, value)?;
        }
    ...
    fn validate_enclosing_method(
        &self,
        attribute_validation_context: &mut AttributeValidationContext,
        c: &Classfile,
        attr: &AttributeEnclosingType,
        encm: &EnclosingMethod,
    ) -> Result<(), ClassfileError> {
    ...
    fn validate_code(
        ...
        for attribute in attributes {
            self.validate_attribute(
                &mut new_attribute_validation_context,
                attribute,
                c,
                &AttributeEnclosingType::Code(code),
            )?;
