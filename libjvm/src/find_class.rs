...
#[no_mangle]
unsafe extern "system" fn JVM_FindClassFromClassLoader(_env: *mut JNIEnv, name: *const c_char, _init: jboolean, _loader: jobject, _throwError: jboolean) -> jclass {
    dbg!(CStr::from_ptr(name).to_str().unwrap());
    unimplemented!()
}

#[no_mangle]
unsafe extern "system" fn JVM_FindClassFromClass(_env: *mut JNIEnv, name: *const c_char, _init: jboolean, _from: jclass) -> jclass {
    dbg!(CStr::from_ptr(name).to_str().unwrap());
    unimplemented!()
}

#[no_mangle]
unsafe extern "system" fn JVM_FindLoadedClass(env: *mut JNIEnv, _loader: jobject, name: jstring) -> jclass {
    let int_state = get_interpreter_state(env);
    let jvm = get_state(env);
...
unsafe extern "system" fn JVM_FindPrimitiveClass(env: *mut JNIEnv, utf: *const c_char) -> jclass {
    ...
    let (_class_name, _as_str, ptype) = if utf_input == float {
