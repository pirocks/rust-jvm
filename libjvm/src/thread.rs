...
use jvmti_jni_bindings::{
    _jobject, JAVA_THREAD_STATE_BLOCKED, JAVA_THREAD_STATE_NEW, JAVA_THREAD_STATE_RUNNABLE, JAVA_THREAD_STATE_TERMINATED, JAVA_THREAD_STATE_TIMED_WAITING, JAVA_THREAD_STATE_WAITING, jboolean, jclass, jint,
    jintArray, jlong, JNIEnv, jobject,
    jobjectArray, jstring, JVM_Available,
};
...
unsafe extern "system" fn JVM_StartThread(env: *mut JNIEnv, thread: jobject) {
    ...
    jvm.thread_state
        .start_thread_from_obj(jvm, int_state, thread_object, false);
}
...
unsafe extern "system" fn JVM_StopThread(env: *mut JNIEnv, thread: jobject, exception: jobject) {
    ...
    let jvm = get_state(env);
    let int_state = get_interpreter_state(env);
    let target_thread = JavaValue::Object(from_object(jvm, thread))
        .cast_thread()
        .get_java_thread(jvm);
...
unsafe extern "system" fn JVM_IsThreadAlive(env: *mut JNIEnv, thread: jobject) -> jboolean {
    ...
    let int_state = get_interpreter_state(env);
    let java_thread = match JavaValue::Object(from_object(jvm, thread))
        .cast_thread()
...
unsafe extern "system" fn JVM_SuspendThread(env: *mut JNIEnv, thread: jobject) {
    let jvm = get_state(env);
    let int_state = get_interpreter_state(env);
    let java_thread = JavaValue::Object(from_object(jvm, thread))
        .cast_thread()
        .get_java_thread(jvm);
...
#[no_mangle]
unsafe extern "system" fn JVM_IsInterrupted(
    env: *mut JNIEnv,
    thread: jobject,
...
#[no_mangle]
unsafe extern "system" fn JVM_HoldsLock(
    env: *mut JNIEnv,
    threadClass: jclass,
    obj: jobject,
) -> jboolean {
    let int_state = get_interpreter_state(env);
    let jvm = get_state(env);
    let monitor: Arc<Monitor2> = todo!()/*JavaValue::Object(from_object(jvm, obj)).unwrap_normal_object().monitor.clone()*/;
...
unsafe extern "system" fn JVM_GetAllThreads(env: *mut JNIEnv, _dummy: jclass) -> jobjectArray {
    ...
    let jobjects = jvm
        ...
        .map(|java_thread| {
            JavaValue::Object(
                todo!(), /*java_thread.try_thread_object().map(|jthread| jthread.object())*/
            )
        })
        .collect::<Vec<_>>();
    let object_array =
        JavaValue::new_vec_from_vec(jvm, jobjects, CClassName::thread().into()).unwrap_object();
    new_local_ref_public(object_array, int_state)
}

#[no_mangle]
unsafe extern "system" fn JVM_SetNativeThreadName(
    env: *mut JNIEnv,
    jthread: jobject,
    name: jstring,
) {
...
#[no_mangle]
unsafe extern "system" fn JVM_DumpThreads(
    env: *mut JNIEnv,
    threadClass: jclass,
...
#[no_mangle]
unsafe extern "system" fn JVM_GetThreadStateValues(
    env: *mut JNIEnv,
...
#[no_mangle]
unsafe extern "system" fn JVM_GetThreadStateNames(
    env: *mut JNIEnv,
    javaThreadState: jint,
    _values: jintArray,
) -> jobjectArray {
...
unsafe fn GetThreadStateNames_impl(
    env: *mut JNIEnv,
    javaThreadState: i32,
    ...
    let names = match javaThreadState as u32 {
        JAVA_THREAD_STATE_NEW => {
            vec![JString::from_rust(
                jvm,
                int_state,
                Wtf8Buf::from_str("NEW"),
            )?]
        }
        JAVA_THREAD_STATE_RUNNABLE => {
            vec![JString::from_rust(
        ...
        JAVA_THREAD_STATE_BLOCKED => {
            vec![JString::from_rust(
                jvm,
                int_state,
                Wtf8Buf::from_str("BLOCKED"),
            )?]
        }
        JAVA_THREAD_STATE_WAITING => {
            vec![
                JString::from_rust(jvm, int_state, Wtf8Buf::from_str("WAITING.OBJECT_WAIT"))?,
                JString::from_rust(jvm, int_state, Wtf8Buf::from_str("WAITING.PARKED"))?,
            ]
        }
        JAVA_THREAD_STATE_TIMED_WAITING => {
            vec![
                JString::from_rust(jvm, int_state, Wtf8Buf::from_str("TIMED_WAITING.SLEEPING"))?,
                JString::from_rust(
                    jvm,
                    int_state,
                    Wtf8Buf::from_str("TIMED_WAITING.OBJECT_WAIT"),
                )?,
