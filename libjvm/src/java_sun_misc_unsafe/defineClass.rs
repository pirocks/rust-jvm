...
use jvmti_jni_bindings::{
    jbyteArray, jclass, jint, jio_fprintf, JNIEnv, jobject, jstring, JVM_DefineClass,
};
...
#[no_mangle]
unsafe extern "system" fn Java_sun_misc_Unsafe_defineClass(
    env: *mut JNIEnv,
    _the_unsafe: jobject,
    name: jstring,
    bytes: jbyteArray,
    ...
    let mut byte_array = from_object(jvm, bytes)
        .unwrap()
        .unwrap_array()
        .unwrap_byte_array(jvm)
        .iter()
        .map(|byte| *byte as u8)
        .collect::<Vec<_>>(); //todo handle npe
    ...
    let loader_name = if loader != null_mut() {
        JavaValue::Object(from_object(jvm, loader))
            .cast_class_loader()
            .to_jvm_loader(jvm)
    } else {
