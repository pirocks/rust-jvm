...
use rust_jvm_common::compressed_classfile::{
    CMethodDescriptor, CompressedParsedDescriptorType, CPDType,
};
...
#[no_mangle]
unsafe extern "system" fn JVM_AllocateNewObject(
    env: *mut JNIEnv,
    obj: jobject,
    currClass: jclass,
    initClass: jclass,
...
#[no_mangle]
unsafe extern "system" fn JVM_SetClassSigners(
    env: *mut JNIEnv,
    cls: jclass,
...
#[no_mangle]
unsafe extern "system" fn JVM_InvokeMethod(
    env: *mut JNIEnv,
    method: jobject,
    obj: jobject,
    args0: jobjectArray,
    ...
    let signature = string_obj_to_string(
        jvm,
        match method_obj
            .lookup_field(jvm, FieldName::field_signature())
            .unwrap_object()
        {
            None => return throw_npe(jvm, int_state),
            Some(method_name) => method_name,
    ...
    let target_runtime_class =
        match check_initing_or_inited_class(jvm, int_state, target_class_name.into()) {
            Ok(x) => x,
    ...
    let MethodDescriptor {
        parameter_types,
        return_type,
    } = parse_method_descriptor(&signature).unwrap();
    let parsed_md = CMethodDescriptor {
        arg_types: parameter_types
            .into_iter()
            .map(|ptype| CPDType::from_ptype(&ptype, &jvm.string_pool))
            .collect_vec(),
    ...
    let is_virtual = !target_runtime_class
        .view()
        .lookup_method(method_name, &parsed_md)
        .unwrap()
        .is_static();
    if is_virtual {
...
#[no_mangle]
unsafe extern "system" fn JVM_NewInstanceFromConstructor(
    env: *mut JNIEnv,
    c: jobject,
    args0: jobjectArray,
) -> jobject {
    ...
    let clazz =
        match class_object_to_runtime_class(&temp_4.cast_class().expect("todo"), jvm, int_state) {
            Some(x) => x,
            None => {
                return throw_npe(jvm, int_state);
            }
    ...
    let MethodDescriptor {
        parameter_types,
        return_type,
    } = parse_method_descriptor(signature_str.as_str()).unwrap();
