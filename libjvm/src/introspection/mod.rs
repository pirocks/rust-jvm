...
use jvmti_jni_bindings::{
    jboolean, jbyteArray, jclass, jint, jio_vfprintf, JNIEnv, jobject, jobjectArray,
    jstring, JVM_ExceptionTableEntryType, jvmtiCapabilities,
};
...
use slow_interpreter::class_objects::{
    get_or_create_class_object, get_or_create_class_object_force_loader,
};
...
unsafe extern "system" fn JVM_GetClassInterfaces(env: *mut JNIEnv, cls: jclass) -> jobjectArray {
    ...
    let interface_vec = match from_jclass(jvm, cls)
        ...
        .map(|interface| {
            let class_obj =
                get_or_create_class_object(jvm, interface.interface_name().into(), int_state)?;
    ...
    let res = Some(jvm.allocate_object(Array(
...
#[no_mangle]
unsafe extern "system" fn JVM_GetDeclaredClasses(
    env: *mut JNIEnv,
    ofClass: jclass,
) -> jobjectArray {
    let jvm = get_state(env);
    ...
    let res_array = match class.view().inner_classes_view() {
        ...
        Some(inner_classes) => inner_classes
            .classes()
            .flat_map(|inner_class| Some(PTypeView::Ref(inner_class.inner_name()?)))
            .collect::<Vec<_>>(),
    }
        .into_iter()
        .map(|ptype| {
            Ok(JavaValue::Object(
                todo!(), /*get_or_create_class_object(jvm, ptype, int_state)?.into()*/
            ))
        })
...
unsafe extern "system" fn JVM_GetClassSignature(env: *mut JNIEnv, cls: jclass) -> jstring {
    ...
    match JString::from_rust(
        ...
        int_state,
        Wtf8Buf::from_string(
            PTypeView::from_compressed(&ptype, &jvm.string_pool).jvm_representation(),
        ),
    ) {
...
unsafe extern "system" fn JVM_GetClassContext(env: *mut JNIEnv) -> jobjectArray {
    let jvm = get_state(env);
    let int_state = get_interpreter_state(env);
    let jclasses = match int_state
        .cloned_stack_snapshot(jvm)
        .into_iter()
        .rev()
        .flat_map(|entry| Some(entry.try_class_pointer()?.cpdtype()))
        .map(|ptype| {
            get_or_create_class_object(jvm, ptype, int_state)
                .map(|elem| JavaValue::Object(elem.into()))
        })
        .collect::<Result<Vec<_>, WasException>>()
    {
    ...
    new_local_ref_public(
        JavaValue::new_vec_from_vec(jvm, jclasses, CClassName::class().into()).unwrap_object(),
        int_state,
    )
}
...
unsafe extern "system" fn JVM_GetClassNameUTF(env: *mut JNIEnv, cb: jclass) -> *const c_char {
    ...
    let int_state = get_interpreter_state(env);
    let jstring =
        match JavaValue::Object(todo!() /*from_jclass(jvm,JVM_GetClassName(env, cb))*/)
            .cast_string()
        {
    ...
    jvm.native
        ...
        .allocate_and_write_vec(
            sketch_string.buf.clone(),
            &mut len as *mut jint,
...
#[no_mangle]
pub unsafe extern "system" fn JVM_GetCallerClass(
    env: *mut JNIEnv,
    ...
    let possibly_class_pointer = stack.find_map(|entry| {
        ...
        if let Some(name) = view.name().try_unwrap_name() {
            if name == CClassName::method()
                && view.method_view_i(entry.method_i()).name() == MethodName::method_invoke()
            {
                return None;
            }
        }
    ...
    let jclass = load_class_constant_by_type(jvm, int_state, &type_)
        .unwrap()
        .unwrap_object();
...
#[no_mangle]
unsafe extern "system" fn JVM_IsSameClassPackage(
    env: *mut JNIEnv,
    class1: jclass,
    class2: jclass,
) -> jboolean {
    let jvm = get_state(env);
    let int_state = get_interpreter_state(env);
    match Reflection::is_same_class_package(
        jvm,
        int_state,
        from_jclass(jvm, class1),
        from_jclass(jvm, class2),
    ) {
        Ok(res) => res,
...
unsafe extern "system" fn JVM_FindClassFromCaller(
    ...
    let p_type = CPDType::Ref(CPRefType::Class(CompressedClassName(
        jvm.string_pool.add_name(name, true),
    )));

    let loader_name = from_object(jvm, loader)
        .map(|loader_obj| {
            JavaValue::Object(loader_obj.into())
                .cast_class_loader()
                .to_jvm_loader(jvm)
        })
        .unwrap_or(LoaderName::BootstrapLoader);

    let class_lookup_result =
        get_or_create_class_object_force_loader(jvm, p_type.clone(), int_state, loader_name);
...
unsafe extern "system" fn JVM_GetClassName(env: *mut JNIEnv, cls: jclass) -> jstring {
    ...
    let full_name =
        PTypeView::from_compressed(&obj.cpdtype(), &jvm.string_pool).class_name_representation();
