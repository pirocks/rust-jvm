...
#[no_mangle]
unsafe extern "system" fn JVM_GetClassDeclaredMethods(
    env: *mut JNIEnv,
    ofClass: jclass,
    publicOnly: jboolean,
) -> jobjectArray {
    ...
    let of_class_obj = JavaValue::Object(from_object(jvm, ofClass))
        .cast_class()
        .expect("todo");
    let int_state = get_interpreter_state(env);
...
fn JVM_GetClassDeclaredMethods_impl(
    jvm: &'gc_life JVMState<'gc_life>,
    int_state: &'_ mut InterpreterStateGuard<'gc_life, 'l>,
    publicOnly: u8,
    loader: LoaderName,
    of_class_obj: JClass<'gc_life>,
) -> Result<jobjectArray, WasException> {
    ...
    let methods = runtime_class_view
        .methods()
        .map(|method| (runtime_class.clone(), method.method_i()));
    ...
    let res = jvm
        .allocate_object(Object::object_array(
            jvm,
            int_state,
            object_array,
            method_class.view().type_(),
        )?)
        .into();
...
#[no_mangle]
unsafe extern "system" fn JVM_GetClassDeclaredConstructors(
    env: *mut JNIEnv,
    ofClass: jclass,
    publicOnly: jboolean,
) -> jobjectArray {
    ...
    let int_state = get_interpreter_state(env);
    let jvm = get_state(env);
    match JVM_GetClassDeclaredConstructors_impl(
        jvm,
        int_state,
        &class_obj.as_runtime_class(jvm),
        publicOnly > 0,
        class_type,
    ) {
        Ok(res) => res,
...
fn JVM_GetClassDeclaredConstructors_impl(
    jvm: &'gc_life JVMState<'gc_life>,
    int_state: &'_ mut InterpreterStateGuard<'gc_life, 'l>,
    class_obj: &RuntimeClass,
    publicOnly: bool,
    class_type: CPDType,
) -> Result<jobjectArray, WasException> {
    ...
    constructors
        .iter()
        .filter(|m| if publicOnly { m.is_public() } else { true })
        .for_each(|m| {
            let constructor =
                Constructor::constructor_object_from_method_view(jvm, int_state, &m).expect("todo");
