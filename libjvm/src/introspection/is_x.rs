...
use jvmti_jni_bindings::{
    jboolean, jclass, jdouble, JNIEnv,
    JVM_Available, jvmtiError_JVMTI_ERROR_CLASS_LOADER_UNSUPPORTED, jvmtiError_JVMTI_ERROR_INVALID_CLASS,
};
...
use slow_interpreter::rust_jni::native_util::{
    from_jclass, from_object, get_interpreter_state, get_state,
};
...
unsafe extern "system" fn JVM_IsInterface(env: *mut JNIEnv, cls: jclass) -> jboolean {
    ...
    let runtime_class = JavaValue::Object(obj)
        .cast_class()
        .expect("todo")
...
unsafe extern "system" fn JVM_IsPrimitiveClass(env: *mut JNIEnv, cls: jclass) -> jboolean {
    let jvm = get_state(env);
    let type_ = JavaValue::Object(from_object(jvm, cls))
        .cast_class()
        .expect("todo")
        .as_type(jvm);
